...
*/
/********************************
 ActiveTransfer DND upload module
*********************************/
/*!
* Requires :
-  jquery-1.7.2.min.js
    ...
    $.CrushFTP.DNDUpload = function (el, options) {
        ...
        base.init = function () {
            ...
            base.$el.find('.files a:not([target^=_blank])').live('click', function (e) {
                e.preventDefault();
                var id = Date.now();
				$('<iframe style="display:none;" id="dndUploadIframe" class="'+id+'"></iframe>').prop('src', this.href).appendTo('body');
                $('.' + id).ready(function(){setTimeout(function(){$('.' + id).remove()},0)});
            });
