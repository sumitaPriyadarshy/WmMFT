...
*/
/***************************
* ActiveTransfer DND integration in WebInterface
****************************/
...
$(function () {
	...
	$.CrushFTP.browserSupportsDND = function()
	{
		...
		else if(is_explorer||(!flag&&(/*@cc_on!@*/false || !!document.documentMode)))
		{
	...
	$.CrushFTP.formatTime = function(secs) {
		...
		if (secs <= 0) {
			...
			if(secsToTime.h !=0)
			{
				remaining = padNumber(secsToTime.h) + ":" + padNumber(secsToTime.m) + ":" + padNumber(secsToTime.s) + getLocalizationKeyExternal("hours");
			}
			else if(secsToTime.m !=0)
			{
				remaining = padNumber(secsToTime.m) + ":" + padNumber(secsToTime.s) + getLocalizationKeyExternal("minutes");
			}
			else if(secsToTime.s !=0)
			{
				remaining = padNumber(secsToTime.s) + getLocalizationKeyExternal("seconds");
			}
		}
		return remaining;
	}
	...
	var overwriteAllLinkBtn = fileRepo.find("button.overwriteAllLinkBtn");
	var clearUploadedAll = fileRepo.find("button.uploadedAll");
	var shareAllUploaded = fileRepo.find("button.shareAllUploadedDlitBtn").prop("disabled", true);
	...
	$.CrushFTP.buildQueueInfo = function(){
		...
		{
			//open the popup
			browseFileButtonPanel.css('display', 'unset');
			var totalSize = 0;
		...
		{
			// closing the popup
			browseFileButtonPanel.css('display', 'none');
			totalFilesText.text("");
			totalSizeText.text("");
			if(!$.CrushFTP.uploadBarHidden)
			{
				...
				viewFileQueue.hide();
				fileQueueInfo.removeClass("ui-widget-content ui-corner-all borderBox");
			}
		}
		$.CrushFTP.bindUploadItemEvents();
		if(fileRepo.find("ul").find("a.overwriteLink:visible").length>0) {
			overwriteAllLinkBtn.prop("disabled", false);
		}
		else
			overwriteAllLinkBtn.prop("disabled", true);

		if(fileRepo.find("li.uploaded").length>0)
			clearUploadedAll.prop("disabled", false);
		else 
    		clearUploadedAll.prop("disabled", true);
		...
		if(fileRepo.find("button.shareUploadDlitBtn:visible").length>0)
			shareAllUploaded.prop("disabled", false);
		else
			shareAllUploaded.prop("disabled", true);
	};
	...
	$.CrushFTP.showIconsForFiles = function()
	{
		fileRepo.find("li:not(.appletItem)").each(function(){
			...
			if(imgSrc.length>0)
				item.find("div.previewIconUpload").find("span").empty().append("<img style='margin-top:5px;' src='"+imgSrc+"'/>");
		});
	};
	...
	$.CrushFTP.bindUploadEvents = function() {
		...
		remvoeErrorAllLinkBtn.unbind().bind("click", function(){
			fileRepo.find("li.hasError").find("button.cancelUpload").trigger("click");
			$(this).hide();
			return false;
		});
		...
		fileRepo.find("button.cancelUploading").unbind().click(function(){
			if($(this).hasClass("uploadedAll"))
			{
				fileRepo.find("li.uploaded").find("button.cancelUpload").trigger("click");
			}
			else
				$(fileRepo.find("li").get().reverse()).each(function(){
					$(this).find("button.cancelUpload").trigger("click");
				});
				fileQueueInfo.removeClass('borderBox');
			return false;
		});

		fileRepo.find("button.startUploading").unbind().click(function(){
		...
		shareAllUploaded.unbind().click(function(){
			var itemsUploaded = fileRepo.find("button.shareUploadDlitBtn");
	...
	$.CrushFTP.getNextQueuedItem = function(notBatchUpload) {
		var startBtn = false;
		var hasBtns = false;
		var errorsWhileUploading=fileRepo.find("span.error").length>0?true:false;
		if(!notBatchUpload)
		{
			fileRepo.find("li:not(.uploaded, .error, .skipped, .ui-state-highlight, .hasError, .warning)").each(function(){
				...
				if(startUploadBtn.length)
				{
					if(!startBtn)
						startBtn = startUploadBtn;
					else
					{
						startUploadBtn.text($(localizations)[0].waiting);
						hasBtns = true;
					}
				}
			});
			if($.CrushFTP.uploadAllQueued && !startBtn && !hasBtns && !errorsWhileUploading)
			{
				if(!$.CrushFTP.UploadProgressing)
				$.growlUI($(localizations)[0].uploadCompletedText, $(localizations)[0].uploadedMultipleFilesText, $.CrushFTP.Options.GrowlTimeout);
	...
	$.CrushFTP.bindUploadItemEvents = function()
	{
		...
		allItems.find("button.cancelUpload").unbind().click(function(){
			var elem = $(this).closest("li");
		...
		allItems.find("button.startUpload").unbind().click(function(e, data){
			if(data && data.allInQueue)
				$.CrushFTP.uploadAllQueued = true;
			var listElem = $(this).closest("li.formProcessed");
			if($.CrushFTP.UploadProgressing)
			{
				$(this).find("a.startUpload").text($(localizations)[0].waiting);
			...
			if(listElem && listElem.length>0)
			{
				...
				else
				{
					...
					$.ajax({
						...
						data: "command=unblockUploads&random=" + rndm,
	...
	$.CrushFTP.storeUploadFormDataToItems = function(formData, elem){
		...
		{
			setTimeout(function() {
				fileRepo.find("button.startUploading").trigger("click");
			}, 100);
		}
	};
	...
	$.CrushFTP.handleWarnings = function() {
		fileRepo.find(".fileExistOnServer").each(function(){
			...
			$(this).hide().after('<button class="dlt-button dlt-button-icon overwrite" style="float:right;margin-top:5px;" title="Overwrite"><a href="javascript:void(0);" class="overwriteLink"></a></button>').parent().find("button.overwrite").click(function(){
				$(this).remove();
				item.removeClass("ui-state-highlight").find("button.startUpload").show().end().find(".error").remove();
				if($.cookie($.CrushFTP.Options.CookieAutoUploadFlag) + "" == "true")
				{
					setTimeout(function() {
						fileRepo.find("button.startUploading").trigger("click");
				...
				overwriteAllLinkBtn.prop("disabled", true);
				return false;
			}).parent().find("div.name").append("<div class='error error-file-display'><i class='dlt-icon-error icon--danger icon--small margin-right-6'></i>"+_escapeHTML($(localizations)[0].error + ":" + $(localizations)[0]['errors.fileExistOnServer'])+"</div>");
		});
		...
		if(bigFiles.length>0 && !(browser=='Netscape' && useragent.lastIndexOf('Chrome/') > 0 && useragent.substr(useragent.lastIndexOf('Chrome/') + 7, 2)>41))
		{
	...
	$.CrushFTP.attachDND = function(o)
	{
		...
		var browseTypeSelector = $("#browseTypeSelector").unbind().click(function(){
			...
			if($(this).hasClass("normal"))
			{
				...
				if(tipInfo)
				{
					var _title = $(localizations)[0].SwitchToNormalUpload;
			...
			else
			{
				...
				if(tipInfo)
				{
					var _title = $(localizations)[0].SwitchToAdvancedUpload;
					$(this).data('thisInfo', {title: _title, zIndex: tipInfo.zIndex});
					cluetipTitle.html(_title);
				}
			}
		...
		function switchUploadType(type, elm)
		{
			...
			else
			{
				...
				if (!$.CrushFTP.appletDNDAdded)
				{
					...
					var loadingStr = getLocalizationKeyExternal("loading");
					elm.parent().append("<span class='spinner' style='display: inline-block;padding-left: 20px;float: left;top: 5px;position: relative;margin: 0px 6px;'>" + loadingStr + "</span>");
					$("#javaAppletDiv").css({
							"right" : "0px",
							"left" : "auto"
						});
				...
				$.CrushFTP.uploadElem = $("#fileupload").crushftp_DNDUpload({
					...
					fileuploadadded : function(e, data)
					{
						...
						fileRepo.find(".dirNoWritable").each(function(){
							$(this).closest("li").find("button.cancelUpload").trigger("click");
						});
		...
		function processAddedFiles(target, data, applet, isDropped)
		{
			...
			$.each(data.files, function (index, file) {
				...
				if(!applet && file.size)
				{
					...
					{
						// CAT-2330 Advanced upload option no longer available, and the popup appears depends on Alert set here
						// We are commenting the part where alert gets added
	    ...
	    fileRepo.bind("visibilityChange", function(){
			if(fileRepo.is(":visible"))
			{
				...
				viewFileQueue.prop('title',$(localizations)[0].Expand);
				fileQueueInfo.removeClass("ui-widget-content ui-corner-all borderBox");
				globalProgressBar.hide();
				if(fileRepo.find("#fileUploadBar").length==0)
				{
					fileRepo.find("div.fileListHeader").after("<div class='fileuploadBarHolder'></div>");
			...
			else
			{
				...
				if(fileUploadBarHolder.find("#fileUploadBar").length==0)
				{
					...
					fileQueueInfo.addClass("borderBox ui-corner-all");
					// if($.CrushFTP.UploadProgressing)
					// globalProgressBar.show();
					// else
					// globalProgressBar.hide();
	...
	{
		if(fileRepo.find("button.shareUploadDlitBtn").length>0)
			shareAllUploaded.prop("disabled", false);
		else
			shareAllUploaded.prop("disabled", true);
		...
		if(!privs || typeof window.shareFile == "undefined")return;
		privs = unescape(privs);
		if(privs.toLowerCase().indexOf("(share)")>=0 && elem.find("button.shareUploadDlitBtn").length==0)
		{
			...
			{
				reuploadLink.parent().before("<button class='dlt-button dlt-button-icon share block shareUploadDlitBtn' style='width:unset;'><a href='javascript:void(0)' class='shareUploadedItem'></a></div>");
				$.CrushFTP.ShowShareOptionForUploadedItem(elem);
				elem.find("button.shareUploadDlitBtn").unbind().click(function(){
					var fileName = unescape(elem.attr("fileName"));
					var shareMethodUploadedItem = $(document).data("shareMethodUploadedItem");
					if(shareMethodUploadedItem && shareMethodUploadedItem.toLowerCase() == "quick")
					{
	...
	$.CrushFTP.processStatusOfUpload = function(elem, status, result)
	{
		...
		if(status == "completed")
		{
			//Remove the error messages once the file upload is successful after retry/server restart
			if(elem.hasClass("hasError")){
			...
			if(elem.hasClass("appletItem"))
			{
				elem.removeClass("uploadProgress").addClass("uploaded").find("a.startUpload").text().show();
				setTimeout(function(){
					elem.removeClass("uploadProgress").addClass("uploaded").find("a.startUpload").text().show();
			...
			{
				elem.removeClass("uploadProgress").addClass("uploaded").find("a.startUpload").text('').show();
				var itemsUploaded = fileRepo.find("button.startUpload");
				if (itemsUploaded.length > 0) {
					itemsUploaded.first().attr("title", $(localizations)[0].reupload);
				}
				$.CrushFTP.ShowShareOptionForUploadedItem(elem);
				if(elem.data("formData"))
					elem.find("a.editForm").parent().show();
			...
			if(progressInfoHistory && progressInfoHistory.avgSpeed && progressInfoHistory.timeElapsed)
			{
				...
				if(progressInfoHistory.avgSpeed == getLocalizationKeyExternal("BrowserUploaderSpeedTimeCalculatingText"))
				{
					elem.find("div.name").append("<div class='uploadNote'>(" + $(localizations)[0].uploadedInLabelText + "" +progressInfoHistory.timeElapsed+")</div>");
				}
				else
				{
					elem.find("div.name").append("<div class='uploadNote'>(" + $(localizations)[0].uploadedInLabelText + "" +progressInfoHistory.timeElapsed+", "+$(localizations)[0].atAvgSpeedOfLabelText +""+progressInfoHistory.avgSpeed+")</div>");
				}
			}
			...
			else
			{
				...
				else
				{
					...
					if(window.noTimeoutUploadedNote)
						$.growlUI($(localizations)[0].uploadCompletedText, $(localizations)[0].uploadedFileText, false, false, true);
					else
						$.growlUI($(localizations)[0].uploadCompletedText, $(localizations)[0].uploadedFileText, $.CrushFTP.Options.GrowlTimeout);
					if(window.hideUploadBarAfterUpload)
					{
						viewFileQueue.addClass("close").trigger("click");
						viewFileQueue.prop('title',$(localizations)[0].Expand);
		...
		else if(status == "error")
        {
			var progressInfo = elem.find("div.progressInfo");
			progressInfo.hide();
            if(!elem.hasClass("hasError") && !elem.hasClass("appletItem") && result && result.message)
            {
                if(result.message.indexOf('Quota Exceeded') != -1){
                        elem.append("<span class='error' style='color:red; display: flex; padding-top: 6px'><i class='dlt-icon-error icon--danger icon--small margin-right-6'></i>"+ _escapeHTML($(localizations)[0].quotaExceeded)+ "</span>");
                }
				else if (result.message.indexOf("File is infected.") != -1) {
					var startUploadButton = elem.find(".startUpload");
					var startUploadIcon = elem.find("#startUploadIcon");
					startUploadButton.prop('disabled', true);
					startUploadIcon.css('display', 'none');
					$.CrushFTP.UploadProgressing = false;
					elem.append("<span class='error' style='color:red; display: flex;'><i class='dlt-icon-error icon--danger icon--small margin-right-6'></i>" + _escapeHTML(result.message) + "</span>");
				}
				 else {
					var startUploadButton = elem.find(".startUpload");
					var startUploadIcon = elem.find("#startUploadIcon");
					startUploadButton.prop('disabled', false);
					startUploadIcon.css('display', 'block');

					elem.append("<span class='error' style='color:red; display: flex;'><i class='dlt-icon-error icon--danger icon--small margin-right-6'></i>" + _escapeHTML(result.message) + "</span>");
				}
            }
			elem.removeClass("uploadProgress").addClass("hasError").find("a.startUpload").text().show();
			if(elem.data("formData"))
				elem.find("a.editForm").parent().show();
			if($.CrushFTP.uploadAllQueued)
			{
		...
		else
		{
			...
			var totalFiles = batchInfo.files;
			//If total number of files is zero, don't refresh the progress info. If this is not done, global progress bar keep incrementing the timer when server restarts during file upload
			if(totalFiles != 0 && !elem.hasClass("skipped"))
			{
				clearTimeout(currentUploadTimeout);
				currentUploadTimeout = setTimeout(function(){
	...
	$.CrushFTP.abortUploadRequest = function(elem){
		...
		$.ajax({
			type: "POST",
			url: $.CrushFTP.Options.ajaxCallURL,
			data: "command=blockUploads&random=" + rndm,
			success: function (response) {
			}
		});
	};
	...
	$.CrushFTP.updateProgressBar = function(elem, isApplet, appletResult){
		...
		if (history.length > 1 && elem.hasClass("paused") == false) {//Calculation and updating progressbar. Calculation of speed, percentages etc.
			...
			if(isApplet && appletResult && appletResult.status)
			{
				...
				{
					progressInfo.find("div.speed").prepend("<span style='color:red; display: flex;'><i class='dlt-icon-error icon--danger icon--small margin-right-6'></i>" + appletResult.status + "</span><br>");
				}
				else
	...
	$.CrushFTP.updateGlobalProgressBar = function(elem, totalFiles, curFile, isApplet, appletResult)
	{
		...
		if (history.length > 1 && elem.hasClass("paused") == false) {//Calculation and updating progressbar. Calculation of speed, percentages etc.
			...
			.find(".uploadInfo").html($(localizations)[0].uploading + "<strong></strong>, " + curFileIndex + " of " + " total " + totalFiles + " file(s)")
			.after(speedInfo).after(timeStampLabel);
			globalProgressBar.find('strong').text(fileName);

			if(isApplet && appletResult && appletResult.status)
			{
				if(appletResult.status.indexOf("ERROR:") >= 0 || appletResult.status.indexOf("WARN:") >= 0)
				{
					globalProgressBar.find(".uploadInfo").append("<br>" + "<span style='color:red; display: flex;'><i class='dlt-icon-error icon--danger icon--small margin-right-6'></i>" + appletResult.status + "</span>");
				}
				else
			...
			globalProgressBar.find("a.skip").unbind().click(function(){
				elem.find("button.cancelUpload").trigger("click");
				return false;
			});

			globalProgressBar.find("a.stop").unbind().click(function(){
				fileRepo.find("button.cancelUploading").trigger("click");
				return false;
			});
		}
	}
	...
	$.CrushFTP.removeGlobalProgressInfo = function(dontHide)
	{
		...
		$("#mainContent").find("button.refreshButton").trigger("click");
	}
	...
	$.CrushFTP.DNDUploadCallback = function(status, data, button, template){
		if(status == "completed")
		{
			...
			button = template.find("button.startUpload");
			if(result && result.responseText)
			{
				...
				if (response.toLowerCase() != "success")
	            {
	                ...
	                template.append("<span class='error'>"+_escapeHTML(response)+"</span>");
	            }
				else
		...
		{
			if((/*@cc_on!@*/false || !!document.documentMode) && $.CrushFTP.fileSize > 4294960000 && ($("#browseTypeSelector").hasClass("normal"))){
				return;
			}
			...
			// var startBtn = template.find("button.startUpload").hide();
		...
		{
			var itemsUploaded = template.find("button.startUpload");
				if (itemsUploaded.length > 0) {
					itemsUploaded.first().attr("title", $(localizations)[0].reupload);
				}
			// template.removeClass("uploadProgress curQueue").addClass("skipped").find("a.startUpload").text($(localizations)[0].reupload).show();
			template.removeClass("uploadProgress curQueue").addClass("skipped");
	...
	$.CrushFTP.getUploadProgress = function(callBack, elem, applet){
		...
		else
		{
			...
			$.ajax({
				type: "POST",
				url: $.CrushFTP.Options.ajaxCallURL,
				data: "command=getUploadStatus&random="+rndm+"&itemName=" + encodeURIComponent(fileName),
				success: function (response) {
					...
					if (responseData.indexOf("PROGRESS:") >= 0) {
						...
						o.status = "completed";
						$('#file-download-template-' + elem.attr('id').split('-')[3]).replaceWith(
								elem);
