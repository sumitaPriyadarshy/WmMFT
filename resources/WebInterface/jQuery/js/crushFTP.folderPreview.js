...
function showFolderPreview(files, item) {
    ...
    if(files && item)
    {
        ...
        for(var i=0; i<files.length;i++)
        {
            ...
            {
                preview+=1;
                parentElem.append("<div><img src='/WebInterface/function/?command=getPreview&random="+rndm+"&size=1&path=" + crushFTPTools.encodeURILocal(unescape(curFile.href_path)) + "&frame=1' /></div>");
                if(preview==4)
                    i=files.length
            }
        }
