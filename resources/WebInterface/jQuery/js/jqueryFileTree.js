...
if (jQuery)(function ($) {
    $.extend($.fn, {
        fileTree: function (o, h) {
            ...
            o.availableFileExtensionImages = ["ai", "csv", "doc", "docx", "file", "gif", "htm", "html", "jar", "jpg", "json", "mov", "mp3", "mp4", "pdf", "php", "png", "ppt", "svg", "txt", "xls", "xlsx", "xml", "zip"]; /*Pagination options*/
            ...
            o.expandedImageURL = o.ImageFilePath + "expanded.svg";
            if (o.collapsedImageURL == undefined)
            //Path to collapsed image - displayed against expanded folder in a grid
            o.collapsedImageURL = o.ImageFilePath + "collapsed.svg";
            if (o.refreshImageURL == undefined)
            //Path to refresj image - displayed against current folder name in a breadcrumbs
            o.refreshImageURL = o.ImageFilePath + "dlt-icon-refresh.svg";
            $(this).each(function () {
                //Template for a header for Grid view
                var headerTemplate = "<thead><tr class='cdk-header-row thead-light'><td class='thSelect theader'><input type='checkbox' class='chkBoxAll edited-dlt-checkbox checkbox-header'></td><td colName='name' class='thName theader header headerSortUp'>" + getLocalizationKey("TreeviewHeaderNameText") + "</td><td colName='root_dir' class='thPath theader listRootDir'>" + getLocalizationKey("TreeviewHeaderPathText") + "</td><td colName='size' class='thSize theader'>" + getLocalizationKey("TreeviewHeaderSizeText") + "</td><td colName='modified' class='thModified theader'>" + getLocalizationKey("TreeviewHeaderModifiedText") + "</td></tr></thead>";
                 ...
                 * Usehash is a boolean, which overrides any path suppliedcccccccccccccccccccccccccccccccccccccccc
                ...
                window.attachCalendarPopup = function(panel) {
                    ...
                    panel.find("input.futureDateField").each(function(){
                        ...
                        $(this).datepicker({ //For date fields which accepts future dates
                            ...
                            showOn: 'both',
                            buttonImage: '/WebInterface/delite/icons/dlt-icon-calender.svg',
                    ...
                    getCustomForm(customFormName, function (data, hasForm, formName, showAlways) {
                        if (hasForm) {
                            ...
                            $("button.submitForm", signupPanel).unbind().click(function (event) {
                                ...
                                messageForm += "&command=selfRegistration&random=" + rndm;
                                var obj = messageForm;
                    ...
                    function continueListingLoad()
                    {
                        ...
                        if (o.customData) { /* Conditions if overrid location to use from location hash*/
                            if ((!$(this).data("initialized") && o.overrideFromHash) || useHash) {
								...
								//CAT-1460:DOM-based Cross-site Scripting, with _escapeJS we will be sanitizing the path
								hashLocation = _escapePathJS(hashLocation);
                                if (hashLocation) {
                                    path = hashLocation;
                                } else {
                                    path = "/";
                            ...
                            //     }
                            // });
                            var loadingIndicator = $("#loadingIndicator");//.dialog('option', 'title', 'Loading');
                            loadingIndicator.dialog('option', 'position', { my: "center", at: "center", of: window });
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
									...
									//check whether it is a blocked path
									if(data.blockPathError){
										data.privs = "";
										$.growlUI(getLocalizationKey("error"), "<span class='nodataFoundErrorMsg'></span>" + data.blockPathError , o.GrowlTimeout, "growlError", o.GrowlWithCloseButton);
									}
                                    // Check if it's temp user and in root directory then set the default permission for folder and files
                                    isRootDir = ("true" === data.isVirtualRoot) ;
                                    if (JSON.parse($(document).data("isTempUser")) && isRootDir) {
                                        data.privs = "(read)(view)(resume)(inherited)";
                                        data.listing = data.listing.map(
                                            function (listing) {
                                              listing.privs = "(read)(view)(resume)(inherited)";
                                              return listing;
                                            }
                                        );
                                    }
                                    ...
                                    if(l) {
                                        ...
                                        showItemsBasedOnData(true, obj,undefined);
                                    }
                                    else
                                    {
                                        showItemsBasedOnData(false, obj,undefined);
                                    }
                                    if (callback) {
                                        try {
                                            callback();
                                ...
                                error: function (XMLHttpRequest, textStatus, errorThrown) {
                                    ...
                                    if (path.lastIndexOf(".zip") == path.length - 4) {
                                        ...
                                        showItemsBasedOnData(false, obj,error);
                                    }
                                    if (!$(document).data("uploadOnly")) {
                                        renderListing(parentElement, locateFolder, useHash, null); //empty the listing.
                                    } else {
                ...
                function showItemsBasedOnData(folderLoaded, obj,error)
                {
                    if(!folderLoaded)
                    {
                        obj.path=_escapeJS(obj.path);
                        var msg = getLocalizationKey("NoFilesFoundGrowlText");
                        if(error)
                        msg = error;
                        $.growlUI(getLocalizationKey("error"), "<span class='nodataFoundErrorMsg'></span>" + crushFTPTools.htmlEncode(msg) + crushFTPTools.decodeURILocal(obj.path), o.GrowlTimeout, "growlError", o.GrowlWithCloseButton);
                    }
                    else
                    {
                ...
                function applyPrivs() {
                    ...
                    if (!_delete) {
                        myMenu.find("button[onclick='javascript:delete_items();']").remove();
                    }
                    if(!_write)
                    {
                        myMenu.find('button[onclick="javascript:performAction(\'cut\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'copy\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'paste\');"]').remove();
                       // myMenu.find("a[href*='updateKeywords']").parent().hide();
                        ...
                        $("#fileQueueInfo").show();
                    if (!_makedir) {
                        myMenu.find('button[onclick="javascript:performAction(\'createFolder\');"]').remove();
                    }
                    if (!_slideshow) {
                        myMenu.find('button[onclick="javascript:performAction(\'slideshowDiv\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'Preview\');"]').remove();
                    }
                    if (!_download) {
                        myMenu.find('button[onclick="javascript:performAction(\'download\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'showBasket\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'cut\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'copy\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'paste\');"]').remove();
                    }
                    if(isRootDir&& $(document).data("isTempUser")==='false'){
                        myMenu.find('button[onclick="javascript:performAction(\'download\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'showBasket\');"]').remove();
                        myMenu.find('button[onclick="javascript:performAction(\'addToBasket\');"]').remove();
                    }
                }
                ...
                function bindGoToBottom(){
                    $(".goToBottom").click(function () {
                        $('html,body').animate({
                            scrollTop: $("#BackToTopLinkText").offset().top
                        }, 1500, false);
                    });
                }

                function bindLabel(){
                    $("#shareDate").html($(localizations)[0].shareDate);
                    $("#shareTime").html($(localizations)[0].shareTime);
                }
                function bindTitles(){
                    $("Button.clearFilterLink", "#filterPanelBasket").prop('title',$(localizations)[0].Clear);
                    $('#uploadPopupCollapseIcon').prop('title',$(localizations)[0].Collapse);
                    $('#uploadPopupCloseIcon').prop('title',$(localizations)[0].closeText);
                    $('#browseFileButtonPanel').prop('title','');
                    $('#size1_action').prop('title',$(localizations)[0].Expand);
                    $('#closeCreateFolder').prop('title', $(localizations)[0].closeText);
                    $('#closeCopyDirectLink').prop('title', $(localizations)[0].closeText);
                    $('#SearchWindowCloseIcon').prop('title', $(localizations)[0].closeText);
                    $('#closesPreferences').prop('title', $(localizations)[0].closeText);
                    $('#closeChangePasswordDialog').prop('title', $(localizations)[0].closeText);
                    $('#clearAdvancedFilter').prop('title',$(localizations)[0].SearchResultClearLinkText);
                    // $('.passwordComplexityTooltip').prop('title',$(localizations)[0].passwordComplexityTooltip);
                }
                
                function bindButtonText(){
                    $('#SearchWindowCancelButtonText').html($(localizations)[0].SearchWindowCancelButtonText);
                    $('#uploadIconCancel').html($(localizations)[0].cancel);
                    $('#startUploadIcon').html($(localizations)[0].StartUpload);
                    $('#currentPasswordLabel').text($(localizations)[0].currentPasswordLabel);
                    $('#newPasswordLabel').text($(localizations)[0].newPasswordLabel);
                    $('#confirmPasswordLabel').text($(localizations)[0].confirmPasswordLabel);
                    const span1= "<span  style='display:flex;color:#e0000e;text-align:left;gap:0.25rem;padding-top:4px;'> <i class='dlt-icon-error icon--small icon--danger icon-style'></i>"+$(localizations)[0].confirmPasswordWithExistingError+"</span>";
                    $('#confirmPasswordWithExistingError').html(span1);
                    const span2= "<span  style='display:flex;color:#e0000e;text-align:left;gap:0.25rem;padding-top:4px;'> <i class='dlt-icon-error icon--small icon--danger icon-style'></i>"+$(localizations)[0].newAndSamePasswordMatchError+"</span>";
                    $("#newAndSamePasswordMatchError").html(span2);
                    const span3= "<span  style='display:flex;color:#e0000e;text-align:left;gap:0.25rem'> <i class='dlt-icon-error icon--small icon--danger icon-style'></i>"+$(localizations)[0].passwordComplexityError+"</span>";
                    $("#passwordComplexityError").html(span3);
                    $('#proceedLabel').text($(localizations)[0].proceedLabel);
                    $('#ShareWindowSendButtonText').html($(localizations)[0].ShareWindowSendButtonText);
                    // $('#ShareWindowSendButtonText').innerText = $(localizations)[0].ShareWindowSendButtonText;
                }

                function bindPlaceHolder(){
                    $("#EnterYourSearchKeyword").prop('placeholder',$(localizations)[0].EnterYourSearchKeyword);
                    $("#txtNewFolder").prop('placeholder',$(localizations)[0].EnterFolderName);
                    $("#filter").prop('placeholder',$(localizations)[0].Search);
                    $("#filterBasket").prop('placeholder',$(localizations)[0].Search);
                }

                function bindCreateFolderCheck(){
                    $("#createFolder").keyup(function (evt) {
                        var evt = (evt) ? evt : ((event) ? event : null);
                        if (evt.keyCode == 27) {
                            $.unblockUI();
                            resetCreateFolder();
                    ...
                    $("#txtNewFolder").keyup(function (evt) {
                        var evt = (evt) ? evt : ((event) ? event : null);
                        if (evt.keyCode == 27) {
                            $.unblockUI();
                            resetCreateFolder();
                            ...
                            if($("#txtNewFolder").val()===''){
                                $('#btnSubmitCreateNew').prop('disabled',true);
                            } else{
                                $('#btnSubmitCreateNew').prop('disabled',false);
                            }
                        }
                    });
                    $("#clearFolderName").click('', function() {
                        $('#btnSubmitCreateNew').prop('disabled',true);
                    });
                ...
                function resetCreateFolder() {
                    $('#chkNavigateAfterMkdir').prop('checked', false);
                    document.getElementById("txtNewFolder").value = '';
                    document.getElementById("txtNewFolderError").style.display = 'none';
                    document.getElementById("txtEmptyNewFolderError").style.display = 'none';
                    document.getElementById("duplicateFolderNameError").style.display = 'none';
                    document.getElementById("folderNameCannotHaveOnlyDotError").style.display = 'none';
                }

                function resetAdvancedSearch() {
                    if(document.getElementById("searchResultNotification").style.display === 'none') {
                        $('#SearchWindowSearchButtonText').prop('disabled',true);
                        $("#date1").prop('checked', false);
                        $("#date2").prop('checked', false);
                        $("#size1").prop('checked', false);
                        $("#type1").prop('checked', false);
                        document.getElementById("EnterYourSearchKeyword").value = '';
                        document.getElementById("date1_value").value = '';
                        document.getElementById("date2_value").value = '';
                        document.getElementById("size1_value").value = '';
                        $("#file").prop('checked', false);
                        $("#folder").prop('checked', false);
                        $("#exact").prop('checked', false);
                        document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'none';
                        document.getElementById("EnterProperBeforeDate").style.display = 'none';
                        document.getElementById("EnterProperAfterDate").style.display = 'none';
                        $select = document.querySelector('#size1_action');
                        $select.value = 'bigger than';
                    }
                }

                function enableSearch() {
                    if (($(EnterYourSearchKeyword).val() !== '' || $(date1_value).val() !== '' || $(date2_value).val() !== '' ||
                        $(size1_value).val() !== '' || file.checked || folder.checked) &&
                        (document.getElementById("EnterProperAfterDate").style.display === 'none' &&
                        document.getElementById("BeforeDateCannotbeLessthanAfter").style.display === 'none' && 
                        document.getElementById("EnterProperBeforeDate").style.display === 'none')) {
                        $('#SearchWindowSearchButtonText').prop('disabled', false);
                    } else {
                        $('#SearchWindowSearchButtonText').prop('disabled', true);
                    }
                }
                ...
                //         var hs;
                //         var els = document.getElementsByClassName('menuLink');
                //         for(i=0; i<els.length; i++) {
                //             if(els[i].href === "javascript:performAction('changePassword');") {
                //                 hs = isOutOfViewport(els[i]);
                //             }
                //         }                        
                //         if(hs) {
                //             document.getElementById("leftMenuNavigator").style.display = 'block'
                //             document.getElementById("rightMenuNavigator").style.display = 'block'
                //         } else {
                //             document.getElementById("leftMenuNavigator").style.display = 'none'
                //             document.getElementById("rightMenuNavigator").style.display = 'none'
                //         }
                //     });            
                // }
                ...
                function bindPasswordChangeCheck(){

                    $("#proceedLabel").attr('aria-disabled', true);
                    $(".clear-icon-existingPassword").click(()=>{
                        $("#existingPassword").val('');
                        validatePasswordFields();
                    })
                    $(".clear-icon-passwordWithExisting").click(()=>{
                        $("#passwordWithExisting").val('');
                        validatePasswordFields();
                    })
                    $(".clear-icon-confirmPasswordWithExisting").click(()=>{
                        $("#confirmPasswordWithExisting").val('');
                        validatePasswordFields();
                    })
                ...
                function validatePasswordFields(){
                    const newPassword = $("#passwordWithExisting").val();
                    let passwordComplexity = $(document).data("passwordComplexity");
                    //     //default password complexity as of now min 8chars,1upper,1lower,1special,1number
                        let message = "";
                        let passwordNotValid = false;
                        let noOfLowerCases = 0;
                        let noOfUpperCases = 0;
                        let noOfDigits = 0;
                        let noOfSpecialCharacters = 0;
                        for (let i = 0; i < newPassword.length; i += 1) {
                          ...
                          else if ("!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~".indexOf(newPassword.charAt(i)) != -1) {
                        ...
                        if (passwordComplexity.minimumLowerCase > noOfLowerCases) {
                          passwordNotValid = true;
                        }
                        else if (passwordComplexity.minimumUpperCase > noOfUpperCases) {
                          passwordNotValid = true;
                        }
                        else if (passwordComplexity.minimumDigits > noOfDigits) {
                          passwordNotValid = true;
                        }
                        else if (passwordComplexity.minimumSpChar > noOfSpecialCharacters) {
                          passwordNotValid = true;
                        }
                        else if (passwordComplexity.allowedMinLength > newPassword.length) {
                          passwordNotValid = true;
                        }
                    if(newPassword!=''){
                        if(passwordNotValid){
                            $("#passwordComplexityError").css('display','block');
                            $("#confirmPasswordWithExistingError").css('display','none');
                            $("#newAndSamePasswordMatchError").css('display','none');
                            $("#proceedLabel").attr('aria-disabled', true);
                            return;
                        }
                        else{
                            $("#passwordComplexityError").css('display','none');
                        }
                    }
                    if($("#existingPassword").val()=='' && $("#confirmPasswordWithExisting").val()=='' && $("#passwordWithExisting").val()==''){
                        $("#confirmPasswordWithExistingError").css('display','none');
                        $("#newAndSamePasswordMatchError").css('display','none');
                        $("#passwordComplexityError").css('display','none');
                        $("#proceedLabel").attr('aria-disabled', true);
                        ...
                        if($("#existingPassword").val()!=''&&($("#existingPassword").val()===$("#confirmPasswordWithExisting").val() || $("#existingPassword").val()===$("#passwordWithExisting").val())){
                            $("#newAndSamePasswordMatchError").css('display','block');
                            $("#confirmPasswordWithExistingError").css('display','none');
                            $("#passwordComplexityError").css('display','none');
                            $("#proceedLabel").attr('aria-disabled', true);
                        } else{
                            if(($("#passwordWithExisting").val()!==$("#confirmPasswordWithExisting").val())){
                                if($("#existingPassword").val()!=''||$("#confirmPasswordWithExisting").val()!=''){
                                $("#confirmPasswordWithExistingError").css('display','block');
                                $("#passwordComplexityError").css('display','none');
                                $("#proceedLabel").attr('aria-disabled', true);
                                }
                                $("#newAndSamePasswordMatchError").css('display','none');
                        ...
                        if($("#existingPassword").val()=='' || $("#confirmPasswordWithExisting").val()=='' || $("#passwordWithExisting").val()==''){
                            ...
                            if($('#newAndSamePasswordMatchError').css('display') == 'none' && $('#confirmPasswordWithExistingError').css('display') == 'none' && $('#passwordComplexityError').css('display') == 'none'){
                                $("#proceedLabel").attr('aria-disabled', "false");
                            }
                        }
                ...
                function bindSearchCheck(){
                    var date1ValidDate = false;
                    var date2ValidDate = false;
                    ...
                    $("#EnterYourSearchKeyword").on('input', function(){
                        enableSearch();
                    });
                    $("#date1_value").on('input', function(){
                        if($(date1_value).val() === '') {
                            $("#date1").prop('checked', false);
                            document.getElementById("EnterProperAfterDate").style.display = 'none';
                            enableSearch();
                        } else {
                            if(isGoodDate($('#date1_value').val())) {
                                date1ValidDate = true;
                                document.getElementById("EnterProperAfterDate").style.display = 'none';
                                $("#date1").prop('checked', true);
                                if($(date2_value).val() !== '' && date2ValidDate) {
                                    var before = new Date($('#date2_value').val());
                                    var after = new Date($('#date1_value').val());
                                    if(after.getTime() < before.getTime()) {
                                        document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'none';
                                        enableSearch();
                                    } else {
                                        document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'block';
                                        $('#SearchWindowSearchButtonText').prop('disabled', true);
                                    }
                                } else {
                                    document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'none';
                                ...
                                date1ValidDate = false;
                                document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'none';
                                document.getElementById("EnterProperAfterDate").style.display = 'block';
                                $('#SearchWindowSearchButtonText').prop('disabled', true);
                            }
                        }
                        
                    });
                    $("#date2_value").on('input', function(){
                        if($(date2_value).val() === '') {
                            $("#date2").prop('checked', false);
                            document.getElementById("EnterProperBeforeDate").style.display = 'none';
                            enableSearch();
                        } else {
                            if(isGoodDate($('#date2_value').val())) {
                                date2ValidDate = true;
                                document.getElementById("EnterProperBeforeDate").style.display = 'none';
                                $("#date2").prop('checked', true);
                                if($(date1_value).val() !== '' && date1ValidDate) {
                                    var before = new Date($('#date2_value').val());
                                    var after = new Date($('#date1_value').val());
                                    if(after.getTime() < before.getTime()) {
                                        document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'none';
                                ...
                                document.getElementById("BeforeDateCannotbeLessthanAfter").style.display = 'none';
                                date2ValidDate = false;
                                document.getElementById("EnterProperBeforeDate").style.display = 'block';
                                $('#SearchWindowSearchButtonText').prop('disabled', true);
                            }
                        }
                    });
                    $("#size1_value").on('input', function(){
                        if($(size1_value).val() === '') {
                            $("#size1").prop('checked', false);
                        } else {
                    ...
                    $("#size1_action").click('', function(){
                        var title= $('#size1_action').prop('title');
                        if(title===$(localizations)[0].Expand){
                            $('#size1_action').prop('title', $(localizations)[0].Collapse);
                        } else {
                            $('#size1_action').prop('title', $(localizations)[0].Expand);
                        }
                    });
                    $('#size1_action').on("blur",function(){
                        $('#size1_action').prop('title', $(localizations)[0].Expand);
                    });
                    ...
                    $("#clearAdvancedFilter").click('', function() {
                        resetAdvancedSearch();
                    });
                    $("#SearchWindowCancelButtonText").click('', function() {
                        resetAdvancedSearch();
                    });
                    $("#SearchWindowCloseIcon").click('', function() {
                        resetAdvancedSearch();
                    });
                    $("#clearInputKeyword").click('', function(){
                        $('#EnterYourSearchKeyword').prop('value','');
                        enableSearch();
                    }); 
                }
                ...
                function renderListing(parentElement, locateFolder, useHash, responseData, searchResult) {
                    ...
                    if (currentView() == "Thumbnail") {
                        ...
                        if (!$("table#filesContainer").data("columnsAdded")) {
                            ...
                            $("table#filesContainer").removeClass('wait').append("<tbody class='file-list-tbody'></tbody>").prepend(headerTemplate);
                        ...
                        if (searchResult) { // Displaying result from search query
                            ...
                            $parent.find("tr.subdirectory").each(function () {
                                ...
                                $(dirSpan).find("img").attr("src", o.expandedImageURL).prop('title',$(localizations)[0].Collapse);
                                var padLeft = 0;
                                if (relLink.parent().css("padding-left")) {
                                    try {
                                        padLeft = parseInt(relLink.parent().css("padding-left").replace("px", ""));
                    ...
                    $("Button.clearFilterLink", "#filterPanel").unbind("click").click(function(e) {
                        e.preventDefault();
                        if ($("#filter").val().length > 0) {
                            var enterEvent = jQuery.Event("keyup");
                            enterEvent.keyCode = 13;
                            $("#filter").val("").keyup().blur();
                            $("#filter").trigger(enterEvent);
                ...
                function bindCountOfFiles() {
                    ...
                    if (currentView() == "Thumbnail") {
                        ...
                        if (window.l && (window.listingInfo.totalItems + window.listingInfo.hiddenItems < l.length) && (window.listingInfo.totalItems + window.listingInfo.hiddenItems) > 0) {
                            var items = l.length;
              ...
              if (fVal.toLowerCase().search("script>") > -1)
                ...
                function bindSelectionMenu() {
                    if ($("button.selectionLink") && $("button.selectionLink").contextMenu) {
                        $("button.selectionLink").prop('title',$(localizations)[0].SelectFilter);
                        $("button.selectionLink").contextMenu({
                            menu: 'selectionMenu'
                ...
                function bindPagingSizeMenu() {
                    $("button.pageSizeSelectionLink").prop('title',$(localizations)[0].Pagination);
                    if ($("button.pageSizeSelectionLink") && $("button.pageSizeSelectionLink").contextMenu) {
                        $("button.pageSizeSelectionLink").prop('title',$(localizations)[0].Pagination);
                        $("button.pageSizeSelectionLink").contextMenu({
                            menu: 'pagingSizeMenu'
                        }, function (action, el, pos, command) {
                ...
                function refreshView() {
                    ...
                    if (currentView() == "Thumbnail") {
                        ...
                        if (disableNameCol) {
                            filesContainer.find("th.columnName,th.thName").hide();
                        } else {
                            filesContainer.find("th.columnName").show();
                        }
                        if (disableSizeCol) {
                            filesContainer.find("th.columnSize,th.thSize").hide();
                        } else {
                            filesContainer.find("th.columnSize").show();
                        }
                        if (disableModifiedCol) {
                            filesContainer.find("th.columnModified, th.thModified").hide();
                        } else {
                            filesContainer.find("th.columnModified, th.thModified").show();
                        }
                        if (disableKeywordsCol) {
                            filesContainer.find("th.columnKeywords,th.thKeywords").hide();
                        } else {
                ...
                function bindSortingToTreeviewGrid() {
                    ...
                    if ($("thead", filesContainer).find("th").eq(window.listingSortColumn)) {
                        window.listingSortColumn = $("thead", filesContainer).find("th").eq(window.listingSortColumn).attr("colName");
                    ...
                    if (!inverse) {
                        $("thead", filesContainer).find("th[colName='" + window.listingSortColumn + "']").addClass("headerSortUp");
                    } else {
                        $("thead", filesContainer).find("th[colName='" + window.listingSortColumn + "']").addClass("headerSortDown");
                    }
                ...
                function bindSortingToBasketTreeviewGrid() {
                    var th = jQuery('#FileBasketList').find('thead > tr > th').addClass("header"),
                        inverseBasket = true;
                    // sort direction
                    ...
                    // select default column to sort
                    if ($("thead", basketFilesContainer).find("th").eq(window.listingSortColumnBasket)) {
                        window.listingSortColumnBasket = $("thead", basketFilesContainer).find("th").eq(window.listingSortColumnBasket).attr("colName");
                    ...
                    if (!inverseBasket) {
                        $("thead", basketFilesContainer).find("th[colName='" + window.listingSortColumnBasket + "']").addClass("headerSortUp");
                    } else {
                        $("thead", basketFilesContainer).find("th[colName='" + window.listingSortColumnBasket + "']").addClass("headerSortDown");
                    }
                    $(".chkBoxAll", basketFilesContainer).unbind().click(function (event) {
                        var chkBox = $(this);
                        if(chkBox.is(":checked")){
                            basketFilesContainer.find(".chkBox").attr("checked", "checked");
                            $("#submitActionBasket").find(".shareBasketItems").prop("disabled",false);
                        }
                        else{
                            basketFilesContainer.find(".chkBox").removeAttr("checked");
                            $("#submitActionBasket").find(".shareBasketItems").prop("disabled",true);
                        }
                ...
                function checkFilesAvailable(fileList) {
                    if ((fileList && fileList.length == 0) || !fileList) {
                        // var ovrlayBGColor = window.noFilesOverlayBGColor || "#e74444";
                        // var overlayTransparency = window.noFilesOverlayTransparency || 0.6;
                        var $elem = $("div#filesListing");
                        $(".noFilesAvailable", $elem).parent().find(".notificationMsg").remove();
                        $(".noFilesAvailable", $elem).before("<div class='notificationMsg'>" + getLocalizationKey("BasketNoFilesAvailableText") + "</div>");
                ...
                function validFileName(fileName) {
                    indexOfLessThan = Math.max(fileName.indexOf('<'), fileName.indexOf('&lt;'));
                    indexOfGreaterThan = Math.max(fileName.indexOf('>'), fileName.indexOf('&gt;'));
                ...
                function bindDragDropFiles()
                {
                    if(window.isCrush7Ready())
                    {
                        ...
                        if(currentView() == "Thumbnail") {
                          ...
                          // As of now DMZ doesn't support Cut/Copy/Paste so disabling Drag-Move support
                            if (isDMZ === "false") {
                                ...
                                drop : function(event, ui){
                                  ...
                                  {
                                      for(var i = 0; i < items.length; i++) {
                                          if(validFileName(items[i])) {
                                            growl("Error :", getLocalizationKey("CannotMoveFileHTMLTagsInName") , true, 3500);
                                            return false;
                                        }
                                    }
                                    if(items.has(target))
                        ...
                        else
                        {
                            ...
                            if(isDMZ === "false"){
                              ...
                              helper: function(e){
                                ...
                                {
                                  for (var i = 0; i < arr.length; i++) {
                                      if(validFileName(arr[i])) {
                                        text = getLocalizationKey("FileContainingHTMLTagName");
                                        list = $("<ul class='dragdropItems ui-corner-all ui-state-highlight'><li class='ui-priority-primary'>"+text+" <span class='targetPath'></span></li></ul>");
                                      } else {
                                            list.append("<li>"+arr[i]+"</li>");
                                        }
                                  };
                    ...
                    if (currentView(basket) == "Thumbnail") {
                        ...
                        elem.each(function () {
                            ...
                            if (opt && opt.preview && opt.preview.toString() != "0") {
                                ...
                                if (!genericIconThumbnail && !itemListed.is(".directoryThumb")) {
                                    $curElem.attr("src", o.ajaxCallURL + "?command=getPreview&random="+rndm+"&size=" + size + "&path=" + crushFTPTools.encodeURILocal(unescape($curElem.parent().attr("rel"))) + "&frame=1").attr("style", o.PreviewIconThumbnailStyle);
                                    if ($.browser.msie)
                                    {
                                ...
                                if (title && title.indexOf("getPreview") < 0) {
                                    $(parentElem).attr("title", "<img src=\"" + o.ajaxCallURL + "?command=getPreview&random="+rndm+"&size=3&path=" + crushFTPTools.encodeURILocal(unescape(relLink.attr("rel"))) +"&frame=1" + "\" border=\"0\" /> <br />" + $(parentElem).attr("title"));
                                }
                            }
                        ...
                        elem.each(function () {
                            ...
                            if ($curElem.attr("preview") != "0" && !genericIconTree) {
                                ...
                                newImg.attr("src", o.ajaxCallURL + "?command=getPreview&random="+rndm+"&size=" + size + "&path=" + crushFTPTools.encodeURILocal(unescape($curElem.find("a").attr("rel"))) + "&frame=1");
                                var relLink = $curElem.find("a").addClass("iconPreviewLink");
                                $curElem.removeClass("fileItem");
                                if (!disableHover) {
                                    $curElem = $curElem.find("a").addClass("vtip");
                                    $curElem.attr("title", "<img src=\"" + o.ajaxCallURL + "?command=getPreview&random="+rndm+"&size=3&path=" + crushFTPTools.encodeURILocal(unescape(relLink.attr("rel"))) + "&frame=1" + "\" border=\"0\" />");
                                    vtip($curElem);
                                }
                            } else {
                ...
                function bindBasketCheckboxEvents() {
                    ...
                    var $chkboxes = $("tr.jqueryFileTree", $("#filesBasket")).not(".file").find(".chkBox").unbind("click").click(function(event) {
                        //get the selectedBasketItems and disable share if none selected
                        var selectedFiles = $("#FileBasketList").find("input.chkBox:visible:checked[shareable]");
                        var $elem = $("div#filesBasket");
                        isBasketOpened = $elem.is(":visible");
                        if(isBasketOpened){
                        ...
                        if (ok) {
                            ...
                            $(t).find('span.expandButton').unbind(o.folderEvent).bind(o.folderEvent, function (event, callback) {
                                ...
                                if ($(this).parent().hasClass('collapsed')) {
                                    ...
                                    var toLocate = $(this).parent().find("span.expandButton").find("img").attr("src", o.expandedImageURL).prop('title',$(localizations)[0].Collapse);
                                    var spanTop = $(document).scrollTop();
                ...
                function getPageSize() {
                    ...
                    if (!CookiePageSize) {
                        if(isValidPageSize(o.CookiePageSize) && isValidPageSize(o.defaultPageSize)) {
                            $.cookie(o.CookiePageSize, o.defaultPageSize, getOptions());
                ...
                // Function to validate the default page size
                function isValidPageSize(size) {
                    if(size === "__WEBINTERFACE_PAGE_SIZE" || size === "100") 
                    return true; 
                    else return false;
                }
                ...
                function handleContextMenuEvents(action, el, pos, command, basket) {
                    ...
                    if (command) {
                        ...
                        try {
                            var commandList = [
                                "javascript:performAction('copyDirectLink',true);",
                                "javascript:performAction('createFolder',true);",
                                "javascript:performAction('download',true);",
                                "javascript:performAction('zip',true);",
                                "javascript:doCopy(true);",
                                "javascript:performAction('paste',true);",
                                "javascript:performAction('cut',true);",
                                "javascript:performAction('delete',true);",
                                "javascript:performAction('addToBasket',true);",
                                "javascript:performAction('showBasket',true);",
                                "javascript:performAction('rename',true);",
                                "javascript:performAction('share',true);"
                            ];
                            if (commandList.includes(command)) {
                                var execute = new Function(command);
                                execute();
                            }
                        } catch (e) {
                ...
                function addToBasket(context, fakeContext) {
                    var elem = false;
                    var exist = false;
                    var fileNameNotContainsHTML = true;
                    var _fileName = "";
                    ...
                    if (window.listingInfo.selectedEverything) // If all items in the list is selected, loop through them and add to the basket
                    {
                        ...
                        for (var i = 0; i < listItems.length; i++) {
                            ...
                            if(validFileName(filePath)) {
                                fileNameNotContainsHTML = false;
                            } else {
                                fileNameNotContainsHTML = true;
                            }
                            filePath = filePath.replace("//", "/");
                            if(_filePath == filePath)return;
                            if (!availableInBasket(ItemsInTheBasket, filePath) && fileNameNotContainsHTMLs) {
                            ...
                            else if(fileNameNotContainsHTML)
                                exist = true;
                        }
                    } else {
                        if (currentView() == "Thumbnail") {
                        ...
                        if (selectedFiles.length > 0) {
                            selectedFiles.each(function (index) {
                                ...
                                if (!availableInBasket(ItemsInTheBasket, file) && fileNameNotContainsHTML) {
                                    ItemsInTheBasket.push({
                                        date : dataRow.date,
                    ...
                    if (ItemsInTheBasket.length > 0 && fileNameNotContainsHTML) {
                        $(document).data(o.BasketDataKey, ItemsInTheBasket);
                        rebuildBasket();
                        if (currentView() != "Thumbnail") {
                        ...
                        if(selectedFiles.length > 0)
                        $.growlUI(getLocalizationKey("AddedToBasket"), '', o.GrowlTimeout);
                    }
                    if (exist) {
                        ...
                        // Show message if file exist in the basket
                        $.growlUI(getLocalizationKey("BasketFileCantAdd"), getLocalizationKey("BasketFileContainsMaliciousCOntent"), o.GrowlTimeout, "growlError");
                ...
                window.rebuildBasket = function(items) {
                    ...
                    else
                    {
                        ...
                        {
                            filesSelectedInBasket.append("<table id='basketFilesContainer' cellpadding='0' cellspacing='0' class='jqueryFilesTable tablesorter tablesorter cdk-table table table-hover table-sm table-row-highlight'><thead><tr class='cdk-header-row thead-light jqueryFileTreeAlt'><th class='thDeleteSelect tr-select-width theader'></th><th class='thSelect theader'><input type='checkBox' class='chkBoxAll edited-dlt-checkbox-basket' /></th><th colName='name' class='thName theader header headerSortUp'>" + getLocalizationKey("TreeviewHeaderNameText") + "</th><th  colName='path' class='thPath theader'>" + getLocalizationKey("TreeviewHeaderPathText") + "</th><th  colName='sizeB' class='thSize theader'>" + getLocalizationKey("TreeviewHeaderSizeText") + "</th><th colName='fulldate' class='thModified theader'>" + getLocalizationKey("TreeviewHeaderModifiedText") + "</th></tr></thead><tbody></tbody></table>");
                            setTimeout(function(){
                    ...
                    for (var i = 0; i < ItemsInTheBasket.length; i++) {
                        if (filesSelectedInBasket.find("a[rel='" + escape(ItemsInTheBasket[i].file) + "']").length == 0) {
                            ...
                            //If atleast one item is found with the search in basket
                            if(newElem[0].className==="jqueryFileTree"){
                               noItemsForFilter=false;
                            }
                    ...
                    if (curView != "Thumbnail")
                    {
                        ...
                        var chkBoxAll = $(".chkBoxAll.edited-dlt-checkbox-basket", filesSelectedInBasket);
                        var chkBox = $(".chkBox.edited-dlt-checkbox-basket", filesSelectedInBasket);
                        if(chkBoxAll.is(":checked"))
                            chkBox.trigger("click").prop("checked", "true");
                        else
                            chkBox.trigger("click").prop("checked","false");
                        // If the search doesnt find any items in basket, uncheck the thead checkbox
                        if(noItemsForFilter){
                    ...
                    if(!basketReopen)
                    {
                        if (!$("#filterBasket").data("eventAdded")) // bind event of keyup on filter input to do live filter if event not added already
                        {
                            ...
                            $("Button.clearFilterLink", "#filterPanelBasket").unbind("click").click(function(e) {
                                e.preventDefault();
                        ...
                        $("#submitActionBasket").find(".clearBasket").unbind().click(function () {
                            $(this).blur();
                            if (confirm(getLocalizationKey("BasketClearAllConfirmMessage"))) {
                                $(document).data(o.BasketDataKey, new Array());
                                $("#submitActionBasket").find(".clearBasket").prop('disabled',true);
                                rebuildBasket();
                            }
                        });
                        //Download link event
                ...
                function generateHTMLForBasketItem(opt)
                {
                    ...
                    if (opt) {
                        ...
                        if (currentView(true) == "Thumbnail") {
                            var rndm = crushFTPTools.getToken();
                            if (opt.type === 'DIR') {
                                var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&random='+rndm+'&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.file, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.file, false, true)+"\"" : "";
                            ...
                            if (opt.type === 'DIR') {
                                var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&random='+rndm+'&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.file, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.file, false, true)+"\"" : "";
                                fileList.push('<tr '+downloadURL+' rootdir="'+escape(opt.path)+'" rel="' + escape(opt.path) + '" preview="'+ opt.preview +'" class="jqueryFileTree directoryThumb'+hideItem+'"> <td class="tr-select-width"><button type="button" class="dlt-button dlt-button-icon dlt-menu-button-icon"><span class="fileRemoveFromBasketTree error" title="'+ $(localizations)[0].BasketRemoveLinkText +'"></span></button></td><td class="columnSelect tr-select-width"><input class="chkBox edited-dlt-checkbox-basket" type="checkbox"/></td><td preview="' + opt.preview + '" class="contextMenuItem directory columnName wordwrap imgTitle" sizeinbytes="'+opt.sizeB+'"><a class="imgLink" title="' + opt.name + '" rel="' + escape(opt.file) + '" href="javascript:void(0);">' + wbr(opt.name, 30) + '</a></td><td class="columnPath">'+opt.path+'</td><td class="columnSize"></td><td class="columnModified">' + opt.date + '</td><td class="columnKeywords wordwrap" title="' + keywords + '">' + crushFTPTools.xmlEncode(keywords) + '</td></tr>');
                            } else if (opt.type === 'FILE') {
                                var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ":" +linkURL+crushFTPTools.htmlEncode(opt.file, false, true)+"\"" : "";
                                fileList.push('<tr '+downloadURL+' rootdir="'+escape(opt.path)+'" rel="' + escape(opt.path) + '" preview="'+ opt.preview +'" class="jqueryFileTree'+hideItem+'"><td class="tr-select-width"><button type="button" class="dlt-button dlt-button-icon dlt-menu-button-icon"><span class="fileRemoveFromBasketTree error" title="'+ $(localizations)[0].BasketRemoveLinkText +'"></span></button></td><td class="columnSelect tr-select-width"><input class="chkBox edited-dlt-checkbox-basket" type="checkbox"/></td><td preview="' + opt.preview + '" class="contextMenuItem fileTR columnName fileItem wordwrap imgTitle"sizeinbytes="'+opt.sizeB+'"><a class="imgLink" title="' + opt.name + '" rel="' + escape(opt.file) + '" href="javascript:void(0);">' + wbr(opt.name, 30) + '</a></td><td class="columnPath">'+opt.path+'</td><td class="columnSize">' + opt.size + '</td><td class="columnModified">' + opt.date + '</td><td class="columnKeywords wordwrap" title="' + keywords + '">' + crushFTPTools.xmlEncode(keywords) + '</td></tr>');
                            }
                        }
                    }
                ...
                function showDownloadBasket(ItemsInTheBasket) {
                    ...
                    if (ItemsInTheBasket) {
                        if (!ItemsInTheBasket.length || ItemsInTheBasket.length == 0) {
                            $(".filesSelectedInBasket", $elem).after("<div class='notificationMsg'>" + getLocalizationKey("BasketNoFilesAvailableText") + "</div>");
                            $(".totalItemCount", $elem).empty();
                            $("#submitActionBasket").find(".download, .shareBasketItems, .clearBasket").prop('disabled',true);
                        } else {
                            $("#submitActionBasket").find(".download, .clearBasket").prop('disabled',false);
                        }
                    } else {
                        $(".filesSelectedInBasket", $elem).after("<div class='notificationMsg'>" + getLocalizationKey("BasketNoFilesAvailableText") + "</div>");
                        $(".totalItemCount", $elem).empty();
                        $("#submitActionBasket").find(".download, .shareBasketItems, .clearBasket").prop('disabled',true);
                        $("#BasketClearAllLinkText").hide();
...
//                      if (s.indexOf("@") > 0 && s.lastIndexOf(".") > s.indexOf("@"))
                ...
                //Validate share form
                function validateShareForm(shareUsernamePermissionsStr) {
                    var validated = true;
                    //Loop through required fields
                    
                    if (shareUsernamePermissionsStr == "(resume)") {
                        $("fieldset", "#hideShareOptionAfterSend").before("<div style='padding:5px;margin:5px 0px; color:red;' class='validationFail'>" + getLocalizationKey("FormValidationNoPermissionText") + "</div>");
                        return false;
                    }
                    ...
                    $("#shareOptionDiv").find(".email").each(function () {
                        var $elm = $(this);
                        // If sendEmail not checked do not validate email components
                        if (!$("#sendEmail").is(":checked")) return;
                        var val = $elm.val();
                        val = $.trim(val);
                        if (val.length > 0) {
                            ...
                            if (!validateEmail(val, true)) {
                                if (($elm.is("#emailCc") || $elm.is("#emailBcc")) && $elm.parent().find("span.requiredField").length > 0) {
                                    $elm.parent().find("span.requiredField").show();
                                } else {                               
                                    $elm.after(" <span class='requiredField'>* <img src='" + o.ImageFilePath + "help.png'  alt='help' title='" + getLocalizationKey("ShareActionEmailValidationFailureHelpToolTip") + "' /></span>");
                                    $elm.next().find("img").cluetip({
                                       splitTitle: '^',
                                ...
                                $elm.parent().find("span.requiredField").hide();
                            }
                        } 
                    });
                    $("#shareOptionDiv").find(".validationFail").remove();
                    if (!validated) {
                        $("fieldset", "#hideShareOptionAfterSend").before("<div style='padding:5px;margin:5px 0px; color:red;' class='validationFail'>" + getLocalizationKey("FormValidationFailText") + "</div>");
                    }
                    return validated;
                }
                ...
                function refreshPreivewIcon(el)
                {
                    ...
                    if (currentView() == "Thumbnail") {
                        ...
                        {
                            _title = _title.replace("command=getPreview", "command=getPreview&random=" + rndm);
                        }
                        else
                        {
                            _title = "<img src=\"" + o.ajaxCallURL + "?command=getPreview&size=3&path=" + crushFTPTools.encodeURILocal(unescape(filePath)) + "&random="+rndm+"&frame=1" + "\" border=\"0\" />" + _title;
                        }
                ...
                window.shareFile = function(context, files, fileNames, multiple, fromBasket) {
                    ...
                    var _fromEmail = $("#emailFrom").val();
                    var _fromEmailDisable = $("#emailFrom").prop('disabled');
                    shareOptionDiv.html(shareOptionDiv.data("html"));
                    $('#emailFrom').prop('value', _fromEmail);
                    $('#emailFrom').prop('disabled', _fromEmailDisable);
                  ...
                  $(".notShareUsername", shareOptionDiv).show();
                    ...
                    if (fileName.length == 0) {
                        $.growlUI(getLocalizationKey("ShareNothingSelectedGrowlText"), getLocalizationKey(""), o.GrowlTimeout, "growlError");
                        return;
                    }
                    ...
                    var directLink = $(".directLink", shareOptionDiv);
                    if (multipleFiles) {
                    ...
                    $("#ShareWindowSendButtonText", shareOptionDiv).click(function () {
                ...
                function performShareFormSubmit(destinationPath, fileName, quickShare, fileNames) {
					if($("#txtdays").is(".box.focus")){
						$('#txtdays').removeClass("focus");
						return;	//if txtdays has focus, don't submit the form
					}
                    ...
                    if (!port) {
                        var hrefMatch = window.location.href.match(/:(\d+)\//);
                        if (hrefMatch) {
                            port = hrefMatch[1];
                        } else {
                    ...
                    if(quickShare)
                    {
                        ...
                        obj = {
                            ...
                            expireDays: $("input#txtdays", "#shareOptionDiv").val(),
                    ...
                    else
                    {
                        ...
                        var shareUsernamePermissionsStr = "(resume)";
                        if ($("#sharePermissionsView").is(":checked")) shareUsernamePermissionsStr += "(view)";
                        if ($("#sharePermissionsDownload").is(":checked")) shareUsernamePermissionsStr += "(read)";
                        if ($("#sharePermissionsDeleteFolder").is(":checked")) shareUsernamePermissionsStr += "(deletedir)";
                        if ($("#sharePermissionsCreateFolder").is(":checked")) shareUsernamePermissionsStr += "(makedir)";
                        if ($("#sharePermissionsRename").is(":checked")) shareUsernamePermissionsStr += "(rename)";
                        if ($("#sharePermissionsUpload").is(":checked")) shareUsernamePermissionsStr += "(write)";
                        if ($("#sharePermissionsDelete").is(":checked")) shareUsernamePermissionsStr += "(delete)";
                        //Validate form first
                        if (!validateShareForm(shareUsernamePermissionsStr)) {
                            $(".mainProcessIndicator").hide();
                            return false;
                        }
                        
                        $("div", "#hideShareOptionAfterSend").hide()
                        $("fieldset", "#hideShareOptionAfterSend").before("<div class='wait' style='background-position: 0px -1px;margin:10px 0px;'>"+getLocalizationKey("loadingIndicatorText")+"</div>");
                        ...
                        obj = {
                            ...
                            {
                                obj.direct_link = $("input#direct_link", "#shareOptionDiv").is(":checked");
                            }
                    }
                    $.ajax({
                        type: "POST",
                        ...
                        success: function (response) {
                            ...
                            try {
                                ...
                                for (var x = 0; x < msgs.length; x++) {
									var userNameElement = IE(msgs[x].getElementsByTagName("username")[0]);
									if(userNameElement != undefined){
										username += userNameElement.textContent;
									}
									var passwordElement = IE(msgs[x].getElementsByTagName("password")[0]);
									if(passwordElement != undefined){
										password += crushFTPTools.decodeURILocal(passwordElement.textContent);
									}
									var messageElement = IE(msgs[x].getElementsByTagName("message")[0]);
									if(messageElement != undefined){
										message += unescape(messageElement.textContent);
									}
									var urlElement = IE(msgs[x].getElementsByTagName("url")[0]);
									if(urlElement != undefined){
										url += unescape(urlElement.textContent);
									}
                                }
                            } catch (ex) {}
                            message = window.localizeServerMessage(message);
                            $("div.wait", "#shareOptionDiv").remove();
                            // $("table", "#shareOptionDiv").next().find("fieldset").hide();
                            $("div","#hideShareOptionAfterSend").hide();
                            ...
                            {
                                if (message.indexOf("ERROR:") == 0) {
                                    $("#hideShareOptionAfterSend", "#shareOptionDiv").before("<div style='line-height:25px; text-align:left; padding-bottom: 24px;'>" + message + "</div><div style='text-align: right;'><button class='dlt-button dlt-button-secondary' data-modal-close type=\"submit\" value=\"OK\" onclick=\"$.unblockUI(); return false;\">Cancel</button>" + "</div>");
                                } else {
                                    if (window.shareWindowNoUserInfoAfterShare) {
                                        $("#hideShareOptionAfterSend", "#shareOptionDiv").before("<div style='line-height:25px; text-align:left; padding-bottom: 24px;'>" + message + "<br/> " + getLocalizationKey("ShareActionCompleteLinkText") + " <a href='" + url + "'>" + url + "</a><br/><br/>/></div><div style='text-align: right;'><button class='dlt-button dlt-button-secondary' data-modal-close type=\"submit\" value=\"OK\" onclick=\"$.unblockUI(); return false;\">Cancel</button>" + "</div>");
                                    }
                                    else {
                                        $("#hideShareOptionAfterSend", "#shareOptionDiv").before("<div style='line-height:25px; text-align:left; padding-bottom: 24px;'>" + message + "<br/>" + getLocalizationKey("ShareActionCompleteUsernameText") + username + "<br/>" + getLocalizationKey("ShareActionCompletePasswordText") + password + "<br/> " + getLocalizationKey("ShareActionCompleteLinkText") + " <a href='" + url + "'>" + url + "</a></div><div style='text-align: right;'><button class='dlt-button dlt-button-secondary' data-modal-close type=\"submit\" value=\"OK\" onclick=\"$.unblockUI(); return false;\">Cancel</button>" + "</div>");
                                    }
                                    if (window.shareWindowNoOpenInEmail) {
                                        $("#hideShareOptionAfterSend", "#shareOptionDiv").next().find("a:first").hide();
                                    }
                                }
                            }
                        },
                ...
                window.submitAction = function(opt, requestType) {
                    ...
                    $("body").append('<form id="crushftp_action" style=\"display:none;\" name="crushftp_action" enctype="multipart/form-data" method="post" target="'+uniqueIFrameID+'">' + '<input type="text" id="command" name="command" value="" />' + '<input type="text" id="path" name="path" value="" />' + '<input type="text" id="paths" name="paths" value="" />' + '<input type="text" id="random" name="random" value="" />' + '</form>');
                    ...
                    for (var key in opt) {
                        if (opt.hasOwnProperty(key)) {
                            if(opt[key]==="download" || opt[key]==="downloadAsZip"){
                                isDownload = true;
                            }
                    ...
                    formToSubmit.submit();
                    uniqueIFrame.on("load",function(event){
                        var error = $(event.target).contents().find("body").find("pre").html();
                        if(isDownload && error){
                            $.growlUI("Download failed", error, o.GrowlTimeout, "growlError");
                        }
                        isDownload=false;
                    })
                }
				...
				var fileNameValidator = /^[-A-Za-z_ªµºÀ-ÖØ-öø-ˁˆ-ˑˠ-ˤˬˮͰ-ʹͶͷͺ-ͽͿΆΈ-ΊΌΎ-ΡΣ-ϵϷ-ҁҊ-ԯԱ-Ֆՙՠ-ֈא-תׯ-ײؠ-يٮٯٱ-ۓەۥۦۮۯۺ-ۼۿܐܒ-ܯݍ-ޥޱߊ-ߪߴߵߺࠀ-ࠕࠚࠤࠨࡀ-ࡘࡠ-ࡪࢠ-ࢴࢶ-ࢽऄ-हऽॐक़-ॡॱ-ঀঅ-ঌএঐও-নপ-রলশ-হঽৎড়ঢ়য়-ৡৰৱৼਅ-ਊਏਐਓ-ਨਪ-ਰਲਲ਼ਵਸ਼ਸਹਖ਼-ੜਫ਼ੲ-ੴઅ-ઍએ-ઑઓ-નપ-રલળવ-હઽૐૠૡૹଅ-ଌଏଐଓ-ନପ-ରଲଳଵ-ହଽଡ଼ଢ଼ୟ-ୡୱஃஅ-ஊஎ-ஐஒ-கஙசஜஞடணதந-பம-ஹௐఅ-ఌఎ-ఐఒ-నప-హఽౘ-ౚౠౡಀಅ-ಌಎ-ಐಒ-ನಪ-ಳವ-ಹಽೞೠೡೱೲഅ-ഌഎ-ഐഒ-ഺഽൎൔ-ൖൟ-ൡൺ-ൿඅ-ඖක-නඳ-රලව-ෆก-ะาำเ-ๆກຂຄງຈຊຍດ-ທນ-ຟມ-ຣລວສຫອ-ະາຳຽເ-ໄໆໜ-ໟༀཀ-ཇཉ-ཬྈ-ྌက-ဪဿၐ-ၕၚ-ၝၡၥၦၮ-ၰၵ-ႁႎႠ-ჅჇჍა-ჺჼ-ቈቊ-ቍቐ-ቖቘቚ-ቝበ-ኈኊ-ኍነ-ኰኲ-ኵኸ-ኾዀዂ-ዅወ-ዖዘ-ጐጒ-ጕጘ-ፚᎀ-ᎏᎠ-Ᏽᏸ-ᏽᐁ-ᙬᙯ-ᙿᚁ-ᚚᚠ-ᛪᛱ-ᛸᜀ-ᜌᜎ-ᜑᜠ-ᜱᝀ-ᝑᝠ-ᝬᝮ-ᝰក-ឳៗៜᠠ-ᡸᢀ-ᢄᢇ-ᢨᢪᢰ-ᣵᤀ-ᤞᥐ-ᥭᥰ-ᥴᦀ-ᦫᦰ-ᧉᨀ-ᨖᨠ-ᩔᪧᬅ-ᬳᭅ-ᭋᮃ-ᮠᮮᮯᮺ-ᯥᰀ-ᰣᱍ-ᱏᱚ-ᱽᲀ-ᲈᲐ-ᲺᲽ-Ჿᳩ-ᳬᳮ-ᳱᳵᳶᴀ-ᶿḀ-ἕἘ-Ἕἠ-ὅὈ-Ὅὐ-ὗὙὛὝὟ-ώᾀ-ᾴᾶ-ᾼιῂ-ῄῆ-ῌῐ-ΐῖ-Ίῠ-Ῥῲ-ῴῶ-ῼⁱⁿₐ-ₜℂℇℊ-ℓℕℙ-ℝℤΩℨK-ℭℯ-ℹℼ-ℿⅅ-ⅉⅎↃↄⰀ-Ⱞⰰ-ⱞⱠ-ⳤⳫ-ⳮⳲⳳⴀ-ⴥⴧⴭⴰ-ⵧⵯⶀ-ⶖⶠ-ⶦⶨ-ⶮⶰ-ⶶⶸ-ⶾⷀ-ⷆⷈ-ⷎⷐ-ⷖⷘ-ⷞⸯ々〆〱-〵〻〼ぁ-ゖゝ-ゟァ-ヺー-ヿㄅ-ㄯㄱ-ㆎㆠ-ㆺㇰ-ㇿ㐀-䶵一-鿯ꀀ-ꒌꓐ-ꓽꔀ-ꘌꘐ-ꘟꘪꘫꙀ-ꙮꙿ-ꚝꚠ-ꛥꜗ-ꜟꜢ-ꞈꞋ-ꞹꟷ-ꠁꠃ-ꠅꠇ-ꠊꠌ-ꠢꡀ-ꡳꢂ-ꢳꣲ-ꣷꣻꣽꣾꤊ-ꤥꤰ-ꥆꥠ-ꥼꦄ-ꦲꧏꧠ-ꧤꧦ-ꧯꧺ-ꧾꨀ-ꨨꩀ-ꩂꩄ-ꩋꩠ-ꩶꩺꩾ-ꪯꪱꪵꪶꪹ-ꪽꫀꫂꫛ-ꫝꫠ-ꫪꫲ-ꫴꬁ-ꬆꬉ-ꬎꬑ-ꬖꬠ-ꬦꬨ-ꬮꬰ-ꭚꭜ-ꭥꭰ-ꯢ가-힣ힰ-ퟆퟋ-ퟻ豈-舘並-龎ﬀ-ﬆﬓ-ﬗיִײַ-ﬨשׁ-זּטּ-לּמּנּסּףּפּצּ-ﮱﯓ-ﴽﵐ-ﶏﶒ-ﷇﷰ-ﷻﹰ-ﹴﹶ-ﻼＡ-Ｚａ-ｚｦ-ﾾￂ-ￇￊ-ￏￒ-ￗￚ-ￜ0-9.\s]+$/;
                //Rename action
                function performRenameAction(context) {
                    ...
                    if (currentView(basket) != "Thumbnail") { //Treeview
                        ...
                        var $curElem = $(el);
                        //MAT-2986
                        var renameExists = $curElem.find(".renamePanel");
                        if(renameExists && renameExists.length >0){
                            return;
                        }
                        var renameTemplate = "<div id='renamePanelID' class='renamePanel'><div style='display: -webkit-inline-box'><div class='dlt-form-group'><input type='text' class='dlt-text-input renameField' id='invalid-rename' /></div><div class='saveButton' title='Save'>" + getLocalizationKey("RenamePanelSaveLinkText") + "</div><div class='cancelButton' title='Cancel'>" + getLocalizationKey("RenamePanelCancelLinkText") + "</div></div><div><span id='renamePanelError' style='display:none;color:red;text-wrap:auto'>" + getLocalizationKey("txtNewFolderError") + "</span></div>" + "<div><span id='txtEmptyNewFolderError' style='display:none;color:red;text-wrap:auto'>" + getLocalizationKey("txtEmptyNewFolderError") + "</span></div>"  + "<div><span id='folderNameCannotHaveOnlyDotError' style='display:none;color:red;text-wrap:auto'>" + getLocalizationKey("folderNameCannotHaveOnlyDotError") + "</span></div>";
                        ...
                        $curElem.find(".saveButton").click(function () {
                            ...
                            if(newName === '.') {
                                document.getElementById("folderNameCannotHaveOnlyDotError").style.display = 'inline';
                                return;
                            }
                            if(newName.endsWith('.')) {
                            ...
                            if(newName === '') {
                                document.getElementById("txtEmptyNewFolderError").style.display = 'inline';
                                document.getElementById("invalid-rename").style.borderColor = '#e0000e!important';
                                document.getElementById("renamePanelError").style.display = 'none';
                                return;
                            } else {
                                document.getElementById("txtEmptyNewFolderError").style.display = 'none';
                            }
                            if ( (newName.match(fileNameValidator)) ) {
                                document.getElementById("renamePanelError").style.display = 'none';
                            } else {
                                document.getElementById("txtEmptyNewFolderError").style.display = 'none';
                                document.getElementById("renamePanelError").style.display = 'inline';
                                document.getElementById("invalid-rename").style.borderColor = '#e0000e!important';
                                return;
                            }
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    if (responseText.length > 0) {
                                        ...
                                        $.growlUI($(localizations)[0].success, getLocalizationKey("RenameSuccessful"), o.GrowlTimeout);
                                        if(basket)
                                        {
                                        ...
                                        if ($curElem.hasClass("directory")) {
                                            ...
                                            function updateLinks(old, update) {
                                                ...
                                                filesContainer.find("tr[rel='" + old + "']").each(function () {
                                                    ...
                                                    curItem.find("a").attr("rel", update + curItem.find("a").closest("td").attr("name")).attr("href", unescape(update + curItem.find("a").closest("td").attr("name"))).attr("title", "<img src=\"" + o.ajaxCallURL + "?command=getPreview&size=3&path=" + crushFTPTools.encodeURILocal(unescape(update + curItem.find("a").closest("td").attr("name"))) + "&random="+rndm+"&frame=1" + "\"" + " border='0' />");
                                                });
                                            }
                ...
                function getActionResponseText(msg) {
                    ...
                    return crushFTPTools.htmlEncode(responseText);
                }
                ...
                function getIconName(text) {
                    switch (text) {
                        ...
                        case 'Createfolder':
                            return 'dlt-icon-add_folder';
                        case 'Download':
                            return 'dlt-icon-download';
                        case 'Userpreferences':
                            return 'dlt-icon-customize';
                        case 'Advancedsearch':
                            return 'dlt-icon-search';
                        case 'Paste':
                            return 'dlt-icon-paste';
                        case 'Upload':
                            return 'dlt-icon-upload';
                        case 'Rename':
                            return 'dlt-icon-edit';
                        case 'Delete':
                            return 'dlt-icon-delete';
                        case 'Share':
                            return 'dlt-icon-share';
                        case 'Addtobasket':
                            return 'dlt-icon-add_basket';
                        case 'Showbasket':
                            return 'dlt-icon-shopping_basket';
                        case 'Logout':
                            return 'dlt-icon-signout';
                        case 'Changepassword':
                            return 'dlt-icon-password';
                    }
                    return 'dlt-icon-refresh';
                }

                function getTitleName(text) {
                    switch (text) {
                        case 'Createfolder':
                ...
                function renderButtons() {
                    ...
                    $("div.mainNavigation").find('button[onclick="javascript:doLogout();"]').parent().remove();
                    ...
                    if (menuList) {
                        ...
                        for (var i = 0; i < menuList.length; i++) {
                            ...
                            if (!currentPrivs || !menuList[i].requiredPriv || currentPrivs.indexOf(menuList[i].requiredPriv) >= 0 || menuList[i].name.toString().indexOf("(custom):")==0)
                            {
                                ...
                                if(localStorage.getItem("language")==='fr' || localStorage.getItem("language")==='ja'){
                                    menuClass = 'menuLinkFrance';
                                }else{
                                    menuClass = 'menuLink';
                                }
                                if(link.endsWith(":external"))
                                {
                                    ...
                                    if(finalText==='Upload'){
                                        // mainMenu.append("<li id='uploadlink'><button class='"+ menuClass + " dlt-button dlt-button-icon dlt-menu-button-icon' type='button' href='" + link + "'><i class='icon--small icon--primary dlt-menu-icon "+getIconName(finalText)+"'></i></button></li>");
                                        // mainMenu.append("<li id='uploadlink'><a target=\"_blank\" class='"+menuClass+"dlt-tab-link' href=\"" + link + "\"> <img style='width: 20px; height: 20px; margin-right: 8px;position: relative;top: 5px;' src='/WebInterface/delite/icons/"+ getIconName(finalText) +".svg'/></a></li>");
                                        mainMenu.append("<li id='uploadlink'> <button class=\"dlt-button dlt-button-icon\" type=\"button\" href=\"" + link + "\"><i class=\"icon--small icon--primary dlt-icon-password-show\"></i></button></li>");
                                    } else
                                    // mainMenu.append("<li id='uploadlink'> <button type=\"button\" href=\"" + link + "\"><i class=\"icon--small icon--primary dlt-icon-password-show\"></i></button></li>");
                                    // mainMenu.append("<li><a target=\"_blank\" class='"+menuClass+" dlt-tab-link' href=\"" + link + "\"> <img style='width: 20px; height: 20px; margin-right: 8px;position: relative;top: 5px;' src='/WebInterface/delite/icons/"+ getIconName(finalText) +".svg'/></a></li>");
                                    mainMenu.append("<li><button class='"+ menuClass + " dlt-tab-link dlt-button dlt-button-icon dlt-menu-button-icon' type='button' onclick="+link+" title='"+getTitleName(finalText)+"'><i class='icon--small icon--primary dlt-menu-icon "+getIconName(finalText)+"'></i></button></li>");
                                }
                                else{
                                    if(finalText==='Upload'){
                                        mainMenu.append("<li id='uploadlink'><button class='"+ menuClass + "  dlt-tab-link dlt-button dlt-button-icon dlt-menu-button-icon' type='button' href='" + link + "' title='"+getTitleName(finalText)+"'><i class='icon--small icon--primary dlt-menu-icon "+getIconName(finalText)+"'></i></button></li>");
                                        // mainMenu.append("<li id='uploadlink'><a class='"+menuClass+" dlt-tab-link' href=\"" + link + "\"> <img style='width: 20px; height: 20px; margin-right: 8px;position: relative;top: 5px;' src='/WebInterface/delite/icons/"+ getIconName(finalText) +".svg'/></a></li>");
                                    } else if(finalText !== "Userpreferences" && finalText !== "Logout" && finalText !== "Login") {
                                        mainMenu.append("<li><button class='"+ menuClass + " dlt-button dlt-button-icon dlt-menu-button-icon' type='button' onclick="+link+" title='"+getTitleName(finalText)+"'><i class='icon--small icon--primary dlt-menu-icon "+getIconName(finalText)+"'></i></button></li>");
                                        // mainMenu.append("<li><a class='"+menuClass+"' href=\"" + link + "\"> <img style='width: 20px; height: 20px; margin-right: 8px;position: relative;top: 5px;' src='/WebInterface/delite/icons/"+ getIconName(finalText) +".svg'/></a></li>");
                                    }
                                }
                            }
                        }
                    ...
                    // hiding the new files 
                    $('#uploadlink').click(function() {
                        var itemsSelected = $("#fileRepo").find("li").length;
                        if(itemsSelected>0){
                            $("#fileQueueInfo").find("#viewFileQueue").trigger("click");
                        } else {
                            $('#fileButton').click();
                        }
                    });
                    ...
                    if ($(document).data("username") == "anonymous") {
                        ...
                        $("ul.topnav,#myMenu").find("a:contains('" + getLocalizationKey("Logout") + "')").parent().remove();
                    } else {
                        $("ul.topnav,#myMenu").find("a:contains('" + getLocalizationKey("Login") + "')").parent().remove();
                    }
                ...
                window.verifyUserStatus = function()
                {
                    ...
                    bindUserName(false, function (response, username) {
                        if (response == "failure") {
                            window.location = "/WebInterface/login.html";//MAT-2920
                        }
                    });
                }

                window.isCloudMode = function(){
                    var rndm = crushFTPTools.getToken();
                    var obj = {
                        command: "isCloudMode",
                        random: rndm
                        ...
                        success: function (msg) {
                            ...
                            if (isInit) {
                                ...
                                var filesBasket = $("#filesBasket").dialog({
									...
									closeText: getLocalizationKey("closeText"),
                                    buttons: {},
                                    resizable: true,
                                    create: function(event, ui) {
                                        // $(event.target).parent().css('position', 'fixed');
                                        var dialogParent = $(event.target).parent();
                                    ...
                                    open:function(event, ui){
                                        ...
                                        filesBasket.find("button").blur();
										$("div#filesBasket.ui-dialog-content.ui-widget-content").focusout(function(){
												$("div#filesBasket.ui-dialog-content.ui-widget-content").parent().css("z-index", "1001");
										});
                                        filesBasket.parent().addClass("downloadBasket");
                                        document.querySelectorAll('.ui-dialog-titlebar-close').forEach(button => {
                                            button.classList.remove("ui-button", "ui-corner-all", "ui-widget");
                                            button.classList.add("dlt-button", "dlt-button-icon", "dlt-menu-button-icon");
                                            button.querySelectorAll('span').forEach(span => span.remove());
                                            const avaiableButton = button.querySelectorAll('i').length;
                                            if(avaiableButton === 0) {
                                                const icon = document.createElement("i");
                                                icon.classList.add("icon--small", "icon--primary", "dlt-menu-icon", "dlt-icon-close"); // Add class to <i>
                                        ...
                                        {
                                            closeButton.after('<a title="Maximize" href="#" role="button" style="display:none;float: right;margin-right: 13px;margin-top: 0px;" class="fullScreen ui-corner-all"><span class="ui-icon ui-icon-arrow-4-diag">Fullscreen</span></a>');
                                            var fullScreenLink = closeButton.parent().find("a.fullScreen").click(function(){
                                                if( $("#filterBasket").val('')!==""){
                                                $("#filterBasket").val('');
                                                $("Button.clearFilterLink", "#filterPanelBasket").click();
                                                ...
                                                else
                                                {
                                                    ...
                                                    filesBasket.find("div.filesSelectedInBasket");
                ...
                window.generateRandomPassword = function(length) {
                    ...
                    randomId += possible.charAt(array[i] % possible.length);
                    return randomId;
                }
                ...
                function bindUserCustomizationInfo(callback) {
                    ...
                    function applyUserInfoToLayout(msg)
                    {
                        ...
                        var tempUser = IE(msg.getElementsByTagName("isTempUser")[0]).textContent;
                        ...
                        for (var x = 0; x < buttons.length; x++) {
                            ...
                            if (IE(buttons[x].getElementsByTagName("for_menu")[0]).textContent == "true") {
                                ...
                                if(opt.name =="(changePassword):Change password"){
                                    if(tempUser == 'true'){
                                        menuListBar.pop();
                                    }
                                }
                        ...
                        $(document).data("isTempUser", IE(msg.getElementsByTagName("isTempUser")[0]).textContent);
                    ...
                    getCustomForm("messageForm", function (data, hasForm, formName, showAlways) {
                        if (hasForm) {
                            ...
                            $("button.submitForm", welcomeForm).unbind().click(function (event) {
                                ...
                                var rndm = crushFTPTools.getToken();
                                messageForm += "&command=messageForm&random=" + rndm;
                                var obj = messageForm;
                ...
                function loadDirectoryPreview() {
                    ...
                    var ok = true;
                    if (!$(document).data("disableDirectoryLogo")) $("div.directoryPreview").empty().append("<img id='directoryPreviewImg' src='" + destinationPath + ".logo.svg' />");
                ...
                window.applyLocalizations = function () {
                    ...
                    if (_loc) {
                        ...
                        $("#filesContainer").find("th.thName").html(getLocalizationKey("TreeviewHeaderNameText"));
                        $("#filesContainer").find("th.thSize").html(getLocalizationKey("TreeviewHeaderSizeText"));
                        $("#filesContainer").find("th.thModified").html(getLocalizationKey("TreeviewHeaderModifiedText"));
                ...
                function applyCustomizations() {
                    ...
                    $("#clearLogs").on('click', function() {
                        $('.filesSelectedInBasket li.uploaded').remove();
                        $("#clearLogs").hide();
                    });
                    var browser = navigator.appName;
                    var useragent = navigator.userAgent;
                    if (browser=='Netscape' && useragent.lastIndexOf('Chrome/') > 0 && useragent.substr(useragent.lastIndexOf('Chrome/') + 7, 2)>41) {
                          $("#browseTypeSelector").hide();
                          $("#DownloadResumeTextLabelBasket").hide();
                          $("#chkDownloadResume").hide();
                          $("#BasketDownloadAdvancedLinkText").removeClass("download advanced");
                          $("#BasketDownloadAdvancedLinkText").hide();
                          $("#UserOptionsWindowDisableCompressionLabelText").hide();
                          $("#autoAppletFlag").hide();
                          $("#UserOptionsWindowLoadJavaAppletLabelText").hide();
                          $("#noCompressionFlag").hide();
                    }
                    if(window.hideCheckBoxColumn)
                    ...
                    for (var i=0;i<customizations.length;i++) //Loop through customizations
                    {
                        ...
                        case "headerColor":
                            ...
                            // $("#headerContent").css("background-color", '#161616');
                            break;
                        case "profileIconColor":
                            // Profile icon color
                            $("#profileIcon").css("background-color", custItem.value);
                            break;
                        case "footer":
                            //Footer html
                        ...
                        case "buttonHoverFontColor":
                            ...
                            $.cssRule({
                                "ul.topnav li button.menuLink:hover": [
                                    ["color", custItem.value + " !important"]
                                    ]
                            });
                            break;
                        ...
                        case "buttonHoverColor":
                            ...
                            $.cssRule({
                                "#mainNavigation button.menuLink:hover": [
                                    ["background-color", custItem.value + " !important"]
                                    ]
                            });
                            break;
                        case "buttonBackgroundImage":
                            ...
                            $.cssRule({
                                "#mainNavigation button.menuLink": [
                        ...
                        case "hideFilterPanel":
                         ...
                         case "hideSelectionLink":
                            if(custItem.value == "true")
                            {
                                $("button.selectionLink").parent().hide();
                            }
                            break;
                        ...
                        case "defaultAdvancedUploader":
                            ...
                            {
                                setTimeout(function(){
                                    $("#browseTypeSelector").removeClass("advanced").addClass("normal").trigger("click").attr("title", $(localizations)[0].SwitchToNormalUpload).removeData('thisInfo').unbind('.cluetip');
                        ...
                        case "searchDefaultModifiedBefore":
                            ...
                            if(val)
                            {
                                ...
                                searchDiv.find("#datepicker-before").val(val);
                            }
                            break;
                        case "searchDefaultModifiedAfter":
                            ...
                            if(val)
                            {
                                ...
                                searchDiv.find("#datepicker-after").val(val);
                            }
                            break;
                ...
                function applyShareCustomizations() {
                    ...
                    var externalPerms = defaultExternalPermissionStr.split(",");
                    for (i = 0; i < externalPerms.length; i++) {
                        var perm = externalPerms[i];
                        switch ( perm ) {
                            case "view":
                                $("#sharePermissionsView").attr ("checked", "checked");
                                break;
                            case "delete":
                                $("#sharePermissionsDelete").attr ("checked", "checked");
                                break;
                            case "rename":
                                $("#sharePermissionsRename").attr ("checked", "checked");
                                break;
                            case "read":
                                $("#sharePermissionsDownload").attr ("checked", "checked");
                                break;
                            case "write":
                                $("#sharePermissionsUpload").attr ("checked", "checked");
                                break;
                            case "createdir":
                                $("#sharePermissionsCreateFolder").attr ("checked", "checked");
                                break;
                            case "deletedir":
                                $("#sharePermissionsDeleteFolder").attr ("checked", "checked");
                                break;
                            default:
                                break;
                        }
                    ...
                    var manageSyncPanel = $("#manageSyncPanel").removeClass("resized").dialog({
                        ...
                        open : function()
                        {
                            ...
                            if(closeButton.parent().find("a.openInNewTab").length==0)
                            {
                                ...
                                closeButton.after('<a title="'+_openLink+'" target="dummyIframe" href="/WebInterface/sync/index.html" role="button" style="float: right;margin-right: 13px;margin-top: 0px;font-size: 12px;font-weight: normal;display: inline-block;width: auto;" class="openInNewTab ui-corner-all"><span style="float:left;" class="ui-icon ui-icon-newwin"></span>'+_openLink+'</a>');
                            ...
                            if (!inValidImg) {
                                ...
                                if(basket)
                                {
                                    ...
                                    if(isMediaItem)
                                    {
                                       ...
                                       row['imageSrc'] = o.ajaxCallURL + '?command=getPreview&size=3&path=' + curItem['file'] + '&random='+rndm+'&frame=1';
                                ...
                                else
                                {
                                    ...
                                    if(isMediaItem)
                                    {
                                       ...
                                       row['imageSrc'] = o.ajaxCallURL + '?command=getPreview&size=3&path=' + curItem['href_path'] + '&random='+rndm+'&frame=1';
                                        ...
                                        playBtn.click(function(event) {
                                            ...
                                            function showPreview()
                                            {
                                                ...
                                                setTimeout(function(){
                                                    ...
                                                    var path = o.downloadURL + "?command=download&random="+rndm+"&mimeType="+mimeType + "&CrushAuth=" + $.cookie("CrushAuth")+"&path="+crushFTPTools.encodeURILocal(_fileName).replace(/\&/g, "%26");
                                                    if(ext == "flv")
                                                    {
                                                        setTimeout(function(){
                                                            content.height(550);
                                                    ...
                                                    else if(ext == "mp3")
                                                    {
                                                        ...
                                                        if(window.useHTML5PlayerWhenAvailable && !!document.createElement('audio').canPlayType && !!document.createElement('audio').canPlayType("audio/mp3"))
                                                        {
                                                            ...
                                                            mPlayer.find("a").click(function(){
                                                                ...
                                                                var _path = o.downloadURL + "?command=download&random="+rndm+"&mimeType="+mimeType + "&CrushAuth=" + $.cookie("CrushAuth")+"&path="+crushFTPTools.encodeURILocal(_fileName).replace(/\&/g, "%26");
                ...
                function showExifInfo(exif, parentElem, path)
                {
                    ...
                    if(items && items.length>0)
                    {
                        ...
                        for(var i=0;i<items.length;i++)
                        {
                            ...
                            exifInfo += "<tr class='exifInfoRow' path='"+path+"' key='"+items[i]+"'' val='"+info+"'><td class='key'><span class='wordwrap'>"+items[i]+" :</span> </td><td class='val wordwrap'><span class='wordwrap'>"+info+"</span></td><td class='edit'>"+editLink+"</td></tr>";
                        }
                ...
                window.performAction = function (elem, context) {
                    var onBlockFn = function () {
                        $('.clear-icon').prop('title', $(localizations)[0].Clear);
                    };
                    ...
                    // }
                        ...
                        $.blockUI({
                            ...
                            css: {
                                ...
                                'background-color': getPopupColor(true),
                                'border': "1px solid " + getPopupColor(),
                                'box-shadow': '0 8px 24px 8px rgb(0 0 0 / 24%)'
                            },
                            onBlock: onBlockFn
                        });
                        ...
                        $.blockUI({
                            ...
                            onBlock: function () {
                                try {
                                    $("#closeShareDiv", "#shareOptionDiv").prop("title", $(localizations)[0].closeText);
                                    $(".clear-icon", "#shareOptionDiv").prop("title", $(localizations)[0].Clear);
                                    $(".ui-datepicker-trigger", "#shareOptionDiv").prop("title", $(localizations)[0].ChooseDate);
                        ...
                        changePasswordExisting(context);
                        return;
                    }
                    else {
                        $elem = $("#" + elem);
                        if($elem && $elem.length>0)
                        {
                            var width = '30%';
                            var margin = '-215px';
                            if(elem == "copyDirectLinkDiv" || elem == "changePasswordDiv")
                            {
                                width = "auto";
                                var val = ($elem.find("input:first").width()/2 + 30);
                                margin = "-" + val + "px";
                                if(elem==="changePasswordDiv"){
                                    width="34%";
                                    $("button.password-show-icon").each(function(){
                                        const img = $(this).find("img");
                                        if(img.attr('src').includes("dlt-icon-password-hide")){
                                        $(this).click();
                                        }
                                    })
                                    $("#proceedLabel").attr("aria-disabled","true");
                ...
                window.doLogout = function (elem) {
                    document.getElementById("accountItem").classList.remove("dlt-dislpay");
                    ...
                    $.ajax({
                        ...
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                            //Security verb tampering request method was not post hence its 405 not allowed
                            if(XMLHttpRequest.status===405){
                              return;
                            }
                            var msg = $(localizations)[0].LogoutError;
                            if(errorThrown){
                                msg= errorThrown
                            }
                            $.cookie("CrushAuth", "", {
                        ...
                        success: function (msg) {
                            ...
                            $.growlUI($(localizations)[0].success, $(localizations)[0].LogoutSuccess, 30000, "", false);
                            $.cookie("CrushAuth", "", {
                                path: '/',
                                expires: -1
                            });
							...
							if(msg != null && msg.documentElement != null && msg.documentElement.textContent != null && msg.documentElement.textContent == "SSO Logged out.")
							{
								document.location = "/WebInterface/logout.html";
							}
							else
							{
							    document.location = "/WebInterface/login.html";
                ...
                //change password
                window.changePasswordExisting = function(elem){
                    getPasswordComplexity();
                    resetPasswordFields();
                    performAction("changePasswordDiv");
                   
                $("#proceedLabel").click(function () {
                    if($("#proceedLabel").attr("aria-disabled") === "true" || $("#proceedLabel").attr("aria-disabled") === true || $("#proceedLabel").attr("disabled")==="true"||  $("#proceedLabel").attr("disabled")===true){
                        return false;
                    }
                    performChangePasswordSubmit(event);
                    return false;
                });
                ...
                function getPasswordComplexity() {
                    ...
                    $.ajax({
                        ...
                        error: function (errorThrown) {
                            ...
                            $.growlUI($(localizations)[0].error,errorThrown, 5000, "growlError", false);
                            $(document).data("passwordComplexity",undefined);
                            $("#loginWheel").hide();
                        },success: function(result){
                            var res = JSON.parse(result);
                            $(document).data("passwordComplexity",res);
                            $('.passwordComplexityTooltip').prop('title',res.description);
                        }
                    });
                }
                ...
                window.performChangePasswordSubmit = function(e){
                    // bindUserName(false);
                    var username = $(document).data("username");
                    var existingPassword=$("#existingPassword").val();
                    var passwordWithExisting=$("#passwordWithExisting").val();
                    var confirmPasswordWithExisting=$("#confirmPasswordWithExisting").val();
                    if (!existingPassword) existingPassword = $("input[name=existingPassword]").val();
                    if (!passwordWithExisting) password = $("input[name=passwordWithExisting]").val();
                    if (!confirmPasswordWithExisting) password = $("input[name=confirmPasswordWithExisting]").val();
                    if(!username){
                        username = '';
                    }
                    var obj = {
                        command: "change.password.withExistingPassword",
                        username: username,
                        oldpassword: encodeURIComponent(existingPassword),
                        newpassword: encodeURIComponent(confirmPasswordWithExisting),
                        encoded: true,
                        random: (function() {
                            var array = new Uint32Array(1);
                            window.crypto.getRandomValues(array);
                    ...
                    $.ajax({
                        ...
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                            ...
                            $("#loginWheel").hide();
                            success_login = false;
                        },success: function(msg, data, request){
                            setTimeout(()=>{location.replace("WebInterface/login.html")},3000);
                            $.growlUI($(localizations)[0].success,msg, 5000, "", false);
                        }
                    });
                }
                ...
                window.startSearch = function (word) {
                    ...
                    {
                        searchDiv.block({
                            message:  getLocalizationKey("SearchProcessNotificationText"),
                            css: {
                                border: 'none',
                                padding: '15px',
                    ...
                    var $_Request = $.ajax({
                        ...
                        data: "random="+rndm+"&path=" + destinationPath + "&" + $("#searchForm").serialize().toString().replace(/=on/g, '=true'),
                        success: function (data) {
                ...
                window.getUploadForm = function(callback, callback2) {
                    ...
                    var $_Request = $.ajax({
                        type: "POST",
                        url: o.ajaxCallURL,
                        data: "command=getCustomForm&random="+rndm+"&form=uploadForm",
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                ...
                function getCustomForm(type, callback, typeName, welcomeFormShown) {
                    ...
                    var obj = {
                        type: "POST",
                        url: o.ajaxCallURL,
                        data: "command=getCustomForm&random="+rndm+"&form=" + type,
                        success: function (response) {
                            var items = $.xml2json(response, true);
                            var html = '';
                ...
                function highlightPasteButton() {
                    if (($(document).data(o.CookieCopiedFiles) && $(document).data(o.CookieCopiedFiles).length > 0)) {
                        $("div.mainNavigation").find("button[title='Paste']").append('<span class="starredMenu"></span>');
                    } else {
                        $("div.mainNavigation")
                        .find("button[title='Paste']")
                        .each(function () {
                            let btn = $(this);
                            let span = btn.find("span.starredMenu");

                            if (span.length) {
                            // If span exists, remove it
                            span.remove();
                            }
                        });
                    }
                    if ($("#browserFileUpload").find("h2").hasClass("spinner")) {
                        $("ul.topnav").find("button[onclick='javascript:browserUploader();'], button[onclick='javascript:performAction(\'upload\');']").addClass("bouncing_loader");
                    }
                }
                ...
                window.doCopy = function (context, flag) {
                    ...
                    $.growlUI(getLocalizationKey(action), msgToShow);
                ...
                window.updateLinksOnSelectionChange = function(basket){
                    ...
                    if(pathsSelected.length>0)
                    {
                        var rndm = crushFTPTools.getToken();
                        var linkURL = unescape(window.location.href.split('#')[0]);
                        var downloadURL = "application/octet-stream:archieve.zip:" +linkURL+"?command=downloadAsZip&random="+rndm+"&zipName=archieve.zip&path="+crushFTPTools.htmlEncode(pathsSelected.join(":"), false, true) + "[@1]&paths="+crushFTPTools.htmlEncode(pathsSelected.join(":")+"[@2]", false, true);
                            ...
                            success: function (data) {
                                ...
                                if(filesExist.length>0)
                                {
                                    ...
                                    fileExistConfirm.dialog({
                                        ...
                                        {
                                            text : "Stop",
                                            class: "dlt-button dlt-button-tertiary",
                                            click : function(){
                                                $(this).dialog("close");
                                            }
                                        },
                                        ...
                                        open: function() {
                                            ...
                                            $(".ui-dialog-buttonset button").each(function () {
                                                $(this).removeClass("ui-button ui-corner-all ui-widget");
                                            });
                                            document.querySelectorAll('.ui-dialog-titlebar-close').forEach(button => {
                                                button.classList.remove("ui-button", "ui-corner-all", "ui-widget");
                                                button.classList.add("dlt-button", "dlt-button-icon", "dlt-menu-button-icon");
                        ...
                        if (destinationPath) { //Target exists in context
                            ...
                            if (copiedItems) {
                                ...
                                if (hasForm) {
                                    ...
                                    var rndm = crushFTPTools.getToken();
                                    pasteForm += "&command=" + action + "&names=" + fileNames + "&destPath=" + destinationPath + "&random=" + rndm;
                                    obj = pasteForm;
                                }
                                if (currentView(basket) != "Thumbnail") {
                                    ...
                                    success: function (response) {
                                        ...
                                        if (responseText.length > 0) {
                                            if (responseText.toString().toLowerCase() == "ok") {
                                                $.growlUI($(localizations)[0].success,getLocalizationKey("PasteSuccessful"), o.GrowlTimeout);
                                                if (curAction == "cut") {
                                                    if(!isDragDrop)
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (responseText.length > 0) {
                                    ...
                                    $.growlUI($(localizations)[0].success,getLocalizationKey("DeleteSuccessful"), o.GrowlTimeout);
                                    $("#filesContainer").find("input.chkBoxAll").removeAttr("checked");
                                    $(".refreshButton").click();
                                    if(basket)
                                    {
                ...
                window.makedir = function (fileName, callback) {
                    ...
                    document.getElementById("duplicateFolderNameError").style.display = 'none';
                    if(fileName === '.') {
                        document.getElementById("folderNameCannotHaveOnlyDotError").style.display = 'block';
                        return;
                    } else {
                        document.getElementById("folderNameCannotHaveOnlyDotError").style.display = 'none';
                    }
                    if (fileName.toString().length == 0) {
                        document.getElementById("txtEmptyNewFolderError").style.display = 'block';
                        return
                    } else {
                        document.getElementById("txtEmptyNewFolderError").style.display = 'none';
                    }
                    if ( (fileName.match(fileNameValidator)) ) {
                        document.getElementById("txtNewFolderError").style.display = 'none';
                    } else {
                        document.getElementById("txtNewFolderError").style.display = 'block';
                        return;
                    }
                        ...
                        success: function (response) {
                            ...
                            if (responseText.length > 0) {
                                if (responseText.toString().toLowerCase() == "ok") {
                                    document.getElementById("txtNewFolder").value = '';
                                    $(".refreshButton").click();
                                    $.growlUI($(localizations)[0].success,getLocalizationKey("CreateSuccessful"), o.GrowlTimeout);
                                    if(callback)
                                    callback(true);
                                } else {
                                    document.getElementById("duplicateFolderNameError").style.display = 'block';
                                    return;
                                }
                            } else {
                                document.getElementById("txtNewFolder").value = '';
                ...
                function checkProgress(progressBar, fileName, callback) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: o.ajaxCallURL,
                        data: "command=getUploadStatus&random="+rndm+"&itemName=" + crushFTPTools.encodeURILocal(fileName),
                        success: function (response) {
                ...
                function reportFileCompleted(progressBar, fileName, callback, cancelled, wasInWaiting, error, errorMsg) {
                    ...
                    if (filesInProgress == 1) {
                        ...
                        $("ul.topnav").find("button[onclick='javascript:browserUploader();'], button[onclick='javascript:performAction(\'upload\');']").removeClass("bouncing_loader");
                ...
                $('select.language-select').on('change', function (evt) {
                    //whenever user selects a language from options, load that corresponding locale
                    ...
                    var msg = getLocalizationKey("BrowserUploaderAlertWhileNavigatingAwayMsgText");
                    popup = false;
                    if($.CrushFTP && $.CrushFTP.UploadProgressing){
                        msg = getLocalizationKey("BrowserUploaderAlertWhileNavigatingAwayMsgText");
                        popup = true;
                    } else if((downloadFilesInProgress > 0)){
                        msg = getLocalizationKey("BrowserDownloadAlertWhileNavigatingAwayMsgText");
                        popup = true;
                    }
                    if(popup){
                        if(confirm(msg)){
                           loadChanges(selectedLanguage);
                        } else{
                            $('select.language-select').val(localStorage.getItem("language") || 'en');
                        }
                    } else {
                       loadChanges(selectedLanguage);
                    }
                    ...
                    bindUserName(false, function (response, username) {
                        if (response == "failure") {
                            ...
                            submitApplet(true, cmd, downloadInfo.destinationPath, progressBar, function () {
                                $("#submitActionBasket").find(".clearBasket").prop('disabled',false);
                            });
                        }
                    });
                }
                ...
                window.loadApplet = function(signed, callback, noNote) {
                    ...
                    if (signed) {
                        ...
                        $("div#browserFileUpload").find(".advancedBrowse").before("<span class='spinner' style='display:inline-block;padding-left:18px;float:right;margin: 7px 0px 0px 10px;'>" + localizations.loading + "</span>");
                ...
                function isChecked(elem) {
                    var allChecked = true;
                    if($(elem).find("TR.jqueryFileTree:visible").length !== 0) {
                ...
                //MAT-18730 getting month/date/time in MM/DD format
                function getFormattedValue(inputDate) {
                    return inputDate < 9 ? '0'+inputDate: '' + inputDate;
                }
                    ...
                    if (curDirItems && curDirItems.length > 0) {
                        ...
                        for (var i = start; i <= maxEntries; i++) {
                            ...
                            if (opt) {
                                ...
                                var fileName = crushFTPTools.htmlEncode(opt.name, false, true);
								var myDate = new Date(parseInt(opt.modified));
                                opt.dateFormatted = getFormattedValue(myDate.getMonth() + 1) + '/' + getFormattedValue(myDate.getDate()) + '/' +myDate.getFullYear() +'\t'+ getFormattedValue(myDate.getHours()) +':'+getFormattedValue(myDate.getMinutes())+':'+getFormattedValue(myDate.getSeconds());
                                if (currentView() == "Thumbnail") {
                                    if (opt.type === 'DIR') {
                                    ...
                                    if (opt.type === 'DIR') {
                                        ...
                                        var rndm = crushFTPTools.getToken();
                                        var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&random='+rndm+'&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.href_path, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.href_path, false, true)+"\"" : "";
                                        fileList.push('<tr '+downloadURL+' index="' + x + '" name="' + opt.name + '" rootdir="' + opt.root_dir + '" modified="' + opt.modified + '" style="display:none;" privs="' + escape(opt.privs) + '" rel="' + escape(unescape(opt.root_dir)) + '" class="dirItemTR jqueryFileTree ' + subdirClass + '" sizeInBytes="' + opt.size + '" size="' + opt.sizeFormatted + '" fulldate="' + opt.modified + '" Date="' + opt.dateFormatted + '" Keywords="' + crushFTPTools.xmlEncode(keywords) + '" preview="' + opt.preview + '"><td class="columnSelect tr-select-width"><input class="chkBox columnSelect edited-dlt-checkbox" type="checkbox"/></td><td preview="' + opt.preview + '" ' + attrs + ' class="directory collapsed columnName '+syncedItemClass+'"><span class="expandButton" rel="' + escape(unescape(opt.href_path)) + '/"><img title="'+$(localizations)[0].Expand+'" src="' + o.collapsedImageURL + '" /></span><a title = "'+ fileName +'" class="folderPreviewLink" href="' + crushFTPTools.htmlEncode(opt.href_path, false, true) + '" rel="' + escape(unescape(opt.href_path)) + '/">' + fileName + '</a></td><td class="columnPath listRootDir">'+crushFTPTools.htmlEncode(opt.root_dir, false, true)+'</td><td class="columnSize" size="' + opt.size + '"></td><td class="columnModified" modified="' + opt.modified + '">' + opt.dateFormatted + '</td><td class="columnKeywords" title="' + crushFTPTools.xmlEncode(keywords) + '"><span>' +  crushFTPTools.xmlEncode(keywords) + '</span></td></tr>');
                                    } else if (opt.type === 'FILE') {
                                        if (keywords.length >= o.MaximumLengthAllowedForKeywordsString) {
                                            keywords = keywords.substr(
                                            0, o.MaximumLengthAllowedForKeywordsString) + "...";
										...
										if(opt.sizeFormatted){
											var sizeFormat = opt.sizeFormatted.substring(opt.sizeFormatted.indexOf(' ')+1);
											//apply localization to file size (bytes, kilo bytes, mega bytes etc)
											if(sizeFormat != ''){
											    if (sizeFormat == 'KiloBytes') {
                                                    opt.sizeFormatted = opt.sizeFormatted.replace(sizeFormat, 'KB');
                                                } else if (sizeFormat == 'MegaBytes') {
                                                    opt.sizeFormatted = opt.sizeFormatted.replace(sizeFormat, 'MB');
                                                } else if (sizeFormat == 'KiloBytes') {
                                                    opt.sizeFormatted = opt.sizeFormatted.replace(sizeFormat, 'Kb');
                                                } else if (sizeFormat == 'GigaBytes') {
                                                    opt.sizeFormatted = opt.sizeFormatted.replace(sizeFormat, 'GB');
                                                } else if (sizeFormat == 'TeraBytes') {
                                                    opt.sizeFormatted = opt.sizeFormatted.replace(sizeFormat, 'TB');
                                                }
											}
										}
                                        fileList.push('<tr '+downloadURL+' index="' + x + '" name="' + opt.name + '" rootdir="' + opt.root_dir + '" style="display:none;"  modified="' + opt.modified + '" privs="' + escape(opt.privs) + '" rel="' + escape(unescape(opt.root_dir)) + '" class="fileItemTR jqueryFileTree ' + subdirClass + '" sizeInBytes="' + opt.size + '" size="' + opt.sizeFormatted + '" fulldate="' + opt.modified + '" Date="' + opt.dateFormatted + '" Keywords="' + crushFTPTools.xmlEncode(keywords) + '" preview="' + opt.preview + '"><td class="columnSelect tr-select-width"><input class="chkBox columnSelect edited-dlt-checkbox" type="checkbox"/></td><td  preview="' + opt.preview + '" ' + attrs + ' class="fileTR columnName fileItem '+syncedItemClass+'"><a title = "'+ fileName +'" class="filePreviewLink" href="' + crushFTPTools.htmlEncode(opt.href_path, false, true) + '" rel="' + escape(unescape(opt.href_path)) + '">' + fileName + '</a></td><td class="columnPath listRootDir">'+crushFTPTools.htmlEncode(opt.root_dir, false, true)+'</td><td class="columnSize" fulldate="' + opt.date + '" sizeInBytes="' + opt.size + '" size="' + opt.size + '">' + opt.sizeFormatted + '</td><td class="columnModified" modified="' + opt.modified + '">' + opt.dateFormatted + '</td><td class="columnKeywords" title="' + crushFTPTools.xmlEncode(keywords) + '"><span>' + crushFTPTools.xmlEncode(keywords) + '</span></td></tr>');
                                    }
                                }
                            }
                        }
                        ...
                        if (np > 1 || o.pagingControlsShowAlways) {
                            ...
                            var appendItem = function (page_id, appendopts) {
                                    ...
                                    if (page_id == current_page) {
                                        var lnk = jQuery("<span class='current'>" + _escapeHTML((appendopts.text)) + "</span>");
                                        if(appendopts.classes == "next" || appendopts.classes == "prev")
                                            lnk = jQuery("<span class='current disable'>" + _escapeHTML((appendopts.text)) + "</span>");
                                    } else {
                ...
                function bindBreadcrumbs() {
                    ...
                    for (var i = dirs.length - 1; i > 0; i--) {
                        var text = _escapeHTML(dirs[i]);
                        if (i < dirs.length - 2) {
                            crumbs = "<li class='breadcrumb-item' title='"+ text+"'><a class='link' href='#" + escape(getLocation(i + 1)) + "'>" + text +"</a><span> / </span></li>" + crumbs;
                        } else {
                            crumbs = "<li class='breadcrumb-item' title='"+ text+"'><a class='link' href='#" + escape(getLocation(i + 1)) + "'>" + text +"</a></li>" + crumbs;
                        }
                        items += 1;
                    }
                    var userName = $(document).data("username") || "Home";
					document.getElementById("userNameDisplay").textContent = userName;
					document.getElementById("accountIcon").title = userName;
                    crumbs = "<li class='home breadcrumb-item' title='"+ userName +"'><a class='link' href='#/' class='userHome'>" + userName +"</a><span> / </span></li>" + crumbs;
                    var crumbsPanel = $("#crumbs");
                    crumbsPanel.parent().find("button.refreshButton").remove();
                    crumbsPanel.html(crumbs).append("<button id='refreshButton' class='refreshButton dlt-button dlt-button-icon dlt-menu-button-icon' type='button' title='Refresh'><i class='icon--small icon--primary dlt-menu-icon dlt-icon-refresh'></i></button>");
                    // crumbsPanel.html(crumbs).append("<span id='refreshButton' class='refreshButton' title='" +$(localizations)[0].Refresh + "'rel='" + _escapeHTML(hashLoc) + "'><img class='breadcrumb-refreshIcon' src='" + o.refreshImageURL + "' /></span>");
                    var refreshButton = $("button.refreshButton").unbind().click(function (event, callback) {
                        window.current_page = o.pagingCurrentPage = 0;
                        var path = "";
                        try{
                    ...
                    $("#crumbs").animate({scrollTop: $("#crumbs").prop("scrollHeight")}, 500);
                }
                ...
                bindButtonText();
                bindPlaceHolder();
                bindCreateFolderCheck();
                bindSearchCheck();
                bindPasswordChangeCheck();
                ...
                $("#loadingIndicator").dialog({
                    ...
                    open: function() {
                        $('body').css('overflow','hidden');
                        $("#mainContent").block({css:{opacity:0}});
...
function stickyRelocate() {
    ...
    if (!isScrolledIntoView($('#mainNavAnchor'))) {
...
var JS_ESCAPE_CHARS = /[&<>"'()]|&amp;|&lt;|&gt;|&quot;|&#x27;/g;

function _escapeJS(text) {
  return _escapeHTML(text).replace(JS_ESCAPE_CHARS, function(match) {
    return JS_ESCAPES[match];
  });
}

var HTML_PATH_ESCAPES = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
    '"': '&quot;',
...
function _escapePathHTML(text) {
    return coerceToString(text).replace(HTML_ESCAPE_CHARS, function (match) {
        return HTML_PATH_ESCAPES[match];
    });
}

function _escapePathJS(text) {
    return _escapePathHTML(text).replace(JS_ESCAPE_CHARS, function (match) {
        return JS_ESCAPES[match];
    });
}